 ...
 * `clawx update`. Use it to verify the completion installation flow
...
\`clawx update\`, \`clawx doctor\`, and \`clawx onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(theme.muted("  1. Remove the '# ClawX Completion' block from your shell profile"));
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`clawx completion --install\` later to enable.`));
    return;
  }
