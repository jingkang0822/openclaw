...
# Build and bundle ClawX into a minimal .app we can open.
# Outputs to dist/ClawX.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/ClawX.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="ClawX"
BUNDLE_ID="${BUNDLE_ID:-ai.clawx.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/clawx/clawx/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/ClawX/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :ClawXBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :ClawXGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/ClawX"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/ClawX"
fi
chmod +x "$APP_ROOT/Contents/MacOS/ClawX"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/ClawX" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/ClawX/Resources/ClawX.icns" "$APP_ROOT/Contents/Resources/ClawX.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/ClawX/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying ClawXKit resources"
CLAWXKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/ClawXKit_ClawXKit.bundle"
if [ -d "$CLAWXKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/ClawXKit_ClawXKit.bundle"
  cp -R "$CLAWXKIT_BUNDLE" "$APP_ROOT/Contents/Resources/ClawXKit_ClawXKit.bundle"
else
  echo "WARN: ClawXKit resource bundle not found at $CLAWXKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running ClawX"
killall -q ClawX 2>/dev/null || true
