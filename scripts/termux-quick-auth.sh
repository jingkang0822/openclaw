...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/clawx/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "ClawX Re-Auth" -c "After getting key, run: ssh $SERVER '~/clawx/scripts/mobile-reauth.sh'" --id clawx-auth
        ;;
    *)
