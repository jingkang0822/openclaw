...
INSTALL_URL="${CLAWX_INSTALL_URL:-https://clawx.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${CLAWX_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${CLAWX_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="clawx"
PACKAGE_NAME="${CLAWX_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (CLAWX_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${CLAWX_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${CLAWX_INSTALL_LATEST_OUT:-}"
fi
