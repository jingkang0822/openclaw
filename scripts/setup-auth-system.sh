#!/bin/bash
# Setup ClawX Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'clawx-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  ClawX message: Send warning via ClawX itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/clawx-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/clawx-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/clawx-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now clawx-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status clawx-auth-monitor.timer"
echo ""
echo "Quick commands:"
