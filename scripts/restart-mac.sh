#!/usr/bin/env bash
# Reset ClawX like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${CLAWX_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="ClawX.app/Contents/MacOS/ClawX"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/ClawX"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/ClawX"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/ClawX"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.clawx.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/clawx-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${CLAWX_RESTART_LOG:-/tmp/clawx-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${CLAWX_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.clawx/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  CLAWX_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node clawx.mjs daemon install --force --runtime node"
      log "  node clawx.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.clawx/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.clawx.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing ClawX instances"
kill_all_clawx
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product ClawX"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/ClawX.app" ]]; then
    APP_BUNDLE="/Applications/ClawX.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/ClawX.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/ClawX.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/ClawX.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set CLAWX_APP_BUNDLE to your installed ClawX.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node clawx.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node clawx.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".clawx", "clawx.json"), "utf8");
        const cfg = JSON.parse(raw);
        const port = cfg && cfg.gateway && typeof cfg.gateway.port === "number" ? cfg.gateway.port : 19789;
        process.stdout.write(String(port));
      } catch {
        process.stdout.write("19789");
      }
    '
  )"
