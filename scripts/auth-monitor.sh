...
#
# Suggested cron: */30 * * * * /home/admin/clawx/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send ClawX notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., clawx-alerts)
...
STATE_FILE="$HOME/.clawx/auth-monitor-state"
...
send_notification() {
    ...
    # Send via ClawX if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use clawx
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via ClawX to $NOTIFY_PHONE..."
            clawx send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! ClawX is down. Run: ssh l36 '~/clawx/scripts/mobile-reauth.sh'" "urgent"
    exit 1
