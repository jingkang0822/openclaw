...
SERVER="${CLAWX_SERVER:-${CLAWX_SERVER:-l36}}"

# Check auth status
termux-toast "Checking ClawX auth..."

STATUS=$(ssh "$SERVER" '$HOME/clawx/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/clawx/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|CLAWX_EXPIRING|CLAWX_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/clawx/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|CLAWX_EXPIRED|CLAWX_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: ClawX won't work until re-auth"
                ;;
        esac
        ;;
