...
ZIP=${1:?"Usage: $0 ClawX-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/clawx/clawx/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^ClawX-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/clawx/clawx/releases/download/v${VERSION}/"}
