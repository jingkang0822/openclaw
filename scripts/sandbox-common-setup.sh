...
BASE_IMAGE="${BASE_IMAGE:-clawx-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-clawx-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=clawx.sandbox=1)
NOTE
