...
# Checks both Claude Code and ClawX auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
CLAWX_AUTH="$HOME/.clawx/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    clawx models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    clawx_status=$(check_clawx_auth 2>/dev/null || true)

    claude_expires=0
    clawx_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        clawx_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        clawx_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$CLAWX_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        --arg bs "$clawx_status" \
        --arg be "$clawx_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            clawx: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$clawx_status" == EXPIRED* ]] || [[ "$clawx_status" == MISSING* ]]; then
        echo "CLAWX_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$clawx_status" == EXPIRING* ]]; then
        echo "CLAWX_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "ClawX Auth (~/.clawx/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$CLAWX_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$CLAWX_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'clawx doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active clawx >/dev/null 2>&1; then
    echo -e "ClawX service: ${GREEN}running${NC}"
else
    echo -e "ClawX service: ${RED}NOT running${NC}"
fi
