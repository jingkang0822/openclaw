...
IMAGE_NAME="clawx-gateway-network-e2e"

PORT="19789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="clawx-net-e2e-$$"
GW_NAME="clawx-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "CLAWX_GATEWAY_TOKEN=$TOKEN" \
	  -e "CLAWX_SKIP_CHANNELS=1" \
	  -e "CLAWX_SKIP_GMAIL_WATCHER=1" \
	  -e "CLAWX_SKIP_CRON=1" \
	  -e "CLAWX_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
