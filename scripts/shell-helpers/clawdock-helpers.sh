#!/usr/bin/env bash
# ClawDock - Docker helpers for ClawX
# Inspired by Simon Willison's "Running ClawX in Docker"
# https://til.simonwillison.net/llms/clawx-docker
#
# Installation:
...
# Common paths to check for ClawX
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/clawx"
  "${HOME}/workspace/clawx"
  "${HOME}/projects/clawx"
  "${HOME}/dev/clawx"
  "${HOME}/code/clawx"
  "${HOME}/src/clawx"
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found ClawX at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/clawx"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ ClawX not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/clawx/clawx.git ~/clawx"
    echo "  cd ~/clawx && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^CLAWX_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d clawx-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart clawx-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f clawx-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec clawx-gateway \
    bash -c 'echo "alias clawx=\"./clawx.mjs\"" > /tmp/.bashrc_clawx && bash --rcfile /tmp/.bashrc_clawx'
}
...
clawdock-cli() {
  _clawdock_compose run --rm clawx-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build clawx-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "CLAWX_GATEWAY_TOKEN=$token" clawx-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" clawx-gateway \
    bash -c './clawx.mjs config set gateway.remote.token "$TOKEN" && ./clawx.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec clawx-gateway \
    bash -c "./clawx.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart clawx-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm clawx-cli dashboard --no-open 2>&1)
  status=$?
...
clawdock-devices() {
  ...
  local output status
  output=$(_clawdock_compose exec clawx-gateway node dist/index.js devices list 2>&1)
  status=$?
  ...
  if [ $status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'clawx config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for ClawX${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (clawx alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to clawx project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.clawx)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'clawx channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'clawx status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.clawx.ai${_CLR_RESET}"
  echo ""
}
