...
const originalConnect = ClawXApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("clawx-app") as ClawXApp;
...
beforeEach(() => {
  ClawXApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__CLAWX_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  ClawXApp.prototype.connect = originalConnect;
  window.__CLAWX_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/clawx/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/clawx");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/clawx/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__CLAWX_CONTROL_UI_BASE_PATH__ = "/clawx";
    const app = mountApp("/clawx/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/clawx");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/clawx/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem("clawx.control.settings.v1", JSON.stringify({ token: "existing-token" }));
    const app = mountApp("/ui/overview?token=abc123");
    await app.updateComplete;
