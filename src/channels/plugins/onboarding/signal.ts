...
function setSignalDmPolicy(cfg: ClawXConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function setSignalAllowFrom(cfg: ClawXConfig, accountId: string, allowFrom: string[]): ClawXConfig {
...
export const signalOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    await prompter.note(
      [
        'Link device with: signal-cli link -n "ClawX"',
        "Scan QR in Signal → Linked Devices",
        `Then run: ${formatCliCommand("clawx gateway call channels.status --params '{\"probe\":true}'")}`,
        `Docs: ${formatDocsLink("/signal", "signal")}`,
      ].join("\n"),
      "Signal next steps",
    );
