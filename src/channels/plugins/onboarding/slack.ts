...
function setSlackDmPolicy(cfg: ClawXConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "ClawX";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for ClawX`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: ClawXConfig, allowFrom: string[]): ClawXConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
