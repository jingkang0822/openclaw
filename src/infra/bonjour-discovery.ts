...
const GATEWAY_SERVICE_TYPE = "_clawx-gw._tcp";
...
function parseDigSrv(stdout: string): { host: string; port: number } | null {
  // dig +short SRV: "0 0 19790 host.domain."
  const line = stdout
    .split("\n")
    .map((l) => l.trim())
    .find(Boolean);
...
function parseDnsSdBrowse(stdout: string): string[] {
  ...
  for (const raw of stdout.split("\n")) {
    ...
    const match = line.match(/_clawx-gw\._tcp\.?\s+(.+)$/);
    if (match?.[1]) {
...
async function discoverWideAreaViaTailnetDns(
  ...
  for (const ptr of ptrs) {
    ...
    const instanceName = ptrName.replace(/\.?_clawx-gw\._tcp\..*$/, "");
