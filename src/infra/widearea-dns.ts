...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.CLAWX_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*clawx-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "clawx";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "clawx");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "clawx-gw");
  ...
  records.push(`_clawx-gw._tcp IN PTR ${instanceLabel}._clawx-gw._tcp`);
  records.push(`${instanceLabel}._clawx-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._clawx-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; clawx-content-hash: ${contentHash}\n${contentBody}`;
}
