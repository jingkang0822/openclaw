...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    process.env.CLAWX_MDNS_HOSTNAME = "test-host";
    process.env.CLAWX_MDNS_HOSTNAME = "test-host";
    ...
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.gatewayPort).toBe("19789");
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("clawx (ClawX)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("clawx");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("clawx.local");
