...
describe("resolveEffectiveHomeDir", () => {
  it("prefers CLAWX_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(path.resolve("/srv/clawx-home"));
  });
  ...
  it("expands CLAWX_HOME when set to ~", () => {
    const env = {
      CLAWX_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for CLAWX_HOME", () => {
    const result = resolveRequiredHomeDir(
      { CLAWX_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when CLAWX_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ CLAWX_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { CLAWX_HOME: "/srv/clawx-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/clawx-home")}/x`);
  });
