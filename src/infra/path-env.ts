...
function candidateBinDirs(opts: EnsureClawXPathOpts): string[] {
  ...
  // Bundled macOS app: `clawx` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "clawx");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/clawx` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "clawx"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `clawx` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureClawXCliOnPath(opts: EnsureClawXPathOpts = {}) {
  if (isTruthyEnvValue(process.env.CLAWX_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.CLAWX_PATH_BOOTSTRAPPED = "1";
