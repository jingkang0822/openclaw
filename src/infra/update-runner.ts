...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "clawx-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "clawx.mjs");
    ...
    const doctorStep = await runStep(
      step("clawx doctor", doctorArgv, gitRoot, { CLAWX_UPDATE_IN_PROGRESS: "1" }),
    );
