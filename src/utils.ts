...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".clawx");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.CLAWX_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$CLAWX_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via CLAWX_STATE_DIR.
