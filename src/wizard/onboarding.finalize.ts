...
export async function finalizeOnboardingWizard(
  ...
  if (!opts.skipHealth) {
    ...
    try {
      ...
      await prompter.note(
        [
          "Docs:",
          "https://docs.clawx.ai/gateway/health",
          "https://docs.clawx.ai/gateway/troubleshooting",
        ].join("\n"),
        "Health check help",
      );
    }
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.clawx.ai/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: ~/.clawx/clawx.json (gateway.auth.token) or CLAWX_GATEWAY_TOKEN.",
        `View token: ${formatCliCommand("clawx config get gateway.auth.token")}`,
        `Generate token: ${formatCliCommand("clawx doctor --generate-gateway-token")}`,
        "Web UI stores a copy in this browser's localStorage (clawx.control.settings.v1).",
        `Open the dashboard anytime: ${formatCliCommand("clawx dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ...
            : "Copy/paste this URL in a browser on this machine to control ClawX.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("clawx dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    ["Back up your agent workspace.", "Docs: https://docs.clawx.ai/concepts/agent-workspace"].join(
      "\n",
    ),
    "Workspace backup",
  );
  ...
  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.clawx.ai/security",
    "Security",
  );
  ...
  await prompter.note(
    hasWebSearchKey
      ? [
          ...
          "Docs: https://docs.clawx.ai/tools/web",
        ].join("\n")
      : [
          "If you want your agent to be able to search the web, you’ll need an API key.",
          "",
          "ClawX uses Brave Search for the `web_search` tool. Without a Brave Search API key, web search won’t work.",
          "",
          "Set it up interactively:",
          `- Run: ${formatCliCommand("clawx configure --section web")}`,
  ...
  await prompter.note(
    'What now: https://clawx.ai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control ClawX."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control ClawX.",
  );
