...
describe("setupOnboardingShellCompletion", () => {
  it("QuickStart: installs without prompting", async () => {
    ...
    const deps = {
      resolveCliName: () => "clawx",
      checkShellCompletionStatus: vi.fn(async () => ({
        shell: "zsh",
        profileInstalled: false,
        cacheExists: false,
        cachePath: "/tmp/clawx.zsh",
    ...
    expect(deps.ensureCompletionCacheExists).toHaveBeenCalledWith("clawx");
    expect(deps.installCompletion).toHaveBeenCalledWith("zsh", true, "clawx");
