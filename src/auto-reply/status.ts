...
type AgentConfig = Partial<NonNullable<NonNullable<ClawXConfig["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.clawx/sessions/<SessionId>.jsonl)
  if (!sessionId) {
    return undefined;
  }
...
const formatVoiceModeLine = (config?: ClawXConfig, sessionEntry?: SessionEntry): string | null => {
  if (!config) {
    return null;
  }
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 ClawX ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: ClawXConfig): string {
  const lines = ["ℹ️ Help", ""];
