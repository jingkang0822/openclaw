...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawx-commands-"));
...
function buildParams(commandBody: string, cfg: ClawXConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
