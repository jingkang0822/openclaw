...
import { scheduleGatewaySigusr1Restart, triggerClawXRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting ClawX in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerClawXRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting ClawX via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
