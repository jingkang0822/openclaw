...
import type { ClawXConfig } from "../config/types.js";
import type {
...
export function isCommandEnabled(cfg: ClawXConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: ClawXConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as ClawXConfig),
...
export function getCommandDetection(_cfg?: ClawXConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: ClawXConfig) {
