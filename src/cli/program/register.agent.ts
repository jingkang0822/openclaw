...
${formatHelpExamples([
  ['clawx agent --to +15555550123 --message "status update"', "Start a new session."],
  ['clawx agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'clawx agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'clawx agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['clawx agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'clawx agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.clawx.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.clawx.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['clawx agents set-identity --agent main --name "ClawX" --emoji "🦞"', "Set name + emoji."],
  ["clawx agents set-identity --agent main --avatar avatars/clawx.png", "Set avatar path."],
  [
    "clawx agents set-identity --workspace ~/.clawx/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "clawx agents set-identity --identity-file ~/.clawx/workspace/IDENTITY.md --agent main",
