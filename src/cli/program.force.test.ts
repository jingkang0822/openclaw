...
describe("gateway --force helpers", () => {
  ...
  it("returns empty list when lsof finds nothing", () => {
    ...
    expect(listPortListeners(19789)).toEqual([]);
  });
  ...
  it("throws when lsof missing", () => {
    ...
    expect(() => listPortListeners(19789)).toThrow(/lsof not found/);
  });
  ...
  it("kills each listener and returns metadata", () => {
    ...
    const killed = forceFreePort(19789);
  ...
  it("retries until the port is free", async () => {
    ...
    const promise = forceFreePortAndWait(19789, {
      timeoutMs: 500,
      intervalMs: 100,
