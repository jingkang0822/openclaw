...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.CLAWX_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.CLAWX_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.CLAWX_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[clawx] ${parsed.error}`);
    process.exit(2);
  }
