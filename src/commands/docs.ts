...
const SEARCH_TOOL = "https://docs.clawx.ai/mcp.SearchClawX";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.clawx.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('clawx docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.clawx.ai/");
      runtime.log(`Search: ${formatCliCommand('clawx docs "your query"')}`);
    }
    return;
  }
