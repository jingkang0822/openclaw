...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".clawx", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: ClawXConfig): boolean {
  const localToken =
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const deprecatedLaunchctlEntries = [
    ["CLAWX_GATEWAY_TOKEN", await getenv("CLAWX_GATEWAY_TOKEN")],
    ["CLAWX_GATEWAY_PASSWORD", await getenv("CLAWX_GATEWAY_PASSWORD")],
    ["CLAWX_GATEWAY_TOKEN", await getenv("CLAWX_GATEWAY_TOKEN")],
    ["CLAWX_GATEWAY_PASSWORD", await getenv("CLAWX_GATEWAY_PASSWORD")],
  ...
  if (deprecatedLaunchctlEntries.length > 0) {
    const lines = [
      ...
      ...deprecatedLaunchctlEntries.map(
        ([key]) => `- \`${key}\` is set; use \`CLAWX_${key.slice(key.indexOf("_") + 1)}\` instead.`,
      ),
    ];
    (deps?.noteFn ?? note)(lines.join("\n"), "Gateway (macOS)");
  }

  const tokenEntries = [["CLAWX_GATEWAY_TOKEN", await getenv("CLAWX_GATEWAY_TOKEN")]] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "CLAWX_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteDeprecatedLegacyEnvVars(
  ...
  const entries = Object.entries(env)
    .filter(
      ([key, value]) => (key.startsWith("CLAWX_") || key.startsWith("CLAWX_")) && value?.trim(),
    )
    .map(([key]) => key);
  if (entries.length === 0) {
    return;
  ...
  const lines = [
    ...
    "- Use CLAWX_* equivalents instead:",
    ...entries.map((key) => {
      const suffix = key.slice(key.indexOf("_") + 1);
      return `  ${key} -> CLAWX_${suffix}`;
    }),
  ];
  (deps?.noteFn ?? note)(lines.join("\n"), "Environment");
}
