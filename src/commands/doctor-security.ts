...
import type { ClawXConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: ClawXConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("clawx security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("clawx configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("clawx config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("clawx doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("clawx config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
