...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "clawx-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.CLAWX_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when CLAWX_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/clawx.json",
      exists: true,
      raw: "{}",
    ...
    process.env.CLAWX_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
