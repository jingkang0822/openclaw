...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveClawXAgentDir: vi.fn().mockReturnValue("/tmp/clawx-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/clawx-agent"),
    ...
    resolveAuthStorePathForDisplay: vi.fn().mockReturnValue("/tmp/clawx-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveClawXAgentDir: mocks.resolveClawXAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveClawXAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/clawx-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/clawx-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/clawx-agent-custom");
