...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveClawXAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("clawx models auth setup-token")}\` or \`${formatCliCommand("clawx configure")}\`.`
          : `Run \`${formatCliCommand("clawx configure")}\` or set an API key env var.`;
