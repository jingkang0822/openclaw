...
import { readConfigFileSnapshot, type ClawXConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: ClawXConfig, model: string): ClawXConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("clawx plugins install")}\`.`,
    );
  }
