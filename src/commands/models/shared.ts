...
import { type ClawXConfig, readConfigFileSnapshot, writeConfigFile } from "../../config/config.js";
...
export async function updateConfig(
  mutator: (cfg: ClawXConfig) => ClawXConfig,
): Promise<ClawXConfig> {
...
export function resolveModelTarget(params: { raw: string; cfg: ClawXConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: ClawXConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("clawx agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
