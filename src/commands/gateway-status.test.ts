...
const loadConfig = vi.fn(() => ({
  gateway: {
    mode: "remote",
    remote: { url: "ws://remote.example:19789", token: "rtok" },
    auth: { token: "ltok" },
  },
}));
const resolveGatewayPort = vi.fn(() => 19789);
  ...
  it("infers SSH target from gateway.remote.url and ssh config", async () => {
    ...
    try {
      ...
      loadConfig.mockReturnValueOnce({
        gateway: {
          mode: "remote",
          remote: { url: "ws://peters-mac-studio-1.sheep-coho.ts.net:19789", token: "rtok" },
        },
      });
  ...
  it("falls back to host-only when USER is missing and ssh config is unavailable", async () => {
    ...
    try {
      ...
      loadConfig.mockReturnValueOnce({
        gateway: {
          mode: "remote",
          remote: { url: "ws://studio.example:19789", token: "rtok" },
        },
      });
