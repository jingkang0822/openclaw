...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "CLAWX_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("CLAWX_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv CLAWX_GATEWAY_TOKEN");
    expect(message).not.toContain("CLAWX_GATEWAY_PASSWORD");
  });
