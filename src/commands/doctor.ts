...
function resolveMode(cfg: ClawXConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  let cfg: ClawXConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("clawx configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("clawx config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("clawx setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("clawx doctor --fix")}" to apply changes.`);
  }
