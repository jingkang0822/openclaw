...
describe("resolveControlUiLinks", () => {
  it("uses customBindHost for custom bind", () => {
    ...
    expect(links.httpUrl).toBe("http://192.168.1.100:19789/");
    expect(links.wsUrl).toBe("ws://192.168.1.100:19789");
  });
  ...
  it("falls back to loopback for invalid customBindHost", () => {
    ...
    expect(links.httpUrl).toBe("http://127.0.0.1:19789/");
    expect(links.wsUrl).toBe("ws://127.0.0.1:19789");
  });
  ...
  it("uses tailnet IP for tailnet bind", () => {
    ...
    expect(links.httpUrl).toBe("http://100.64.0.9:19789/");
    expect(links.wsUrl).toBe("ws://100.64.0.9:19789");
  });
