...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  const initStateDir = async (prefix: string) => {
    ...
    process.env.CLAWX_STATE_DIR = stateDir;
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "clawx-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.CLAWX_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "clawx");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.CLAWX_STATE_DIR = stateDir;
    process.env.CLAWX_CONFIG_PATH = path.join(stateDir, "clawx.json");
