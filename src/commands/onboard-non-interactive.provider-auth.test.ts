...
function captureEnv(): EnvSnapshot {
  return {
    ...
    password: process.env.CLAWX_GATEWAY_PASSWORD,
    disableConfigCache: process.env.CLAWX_DISABLE_CONFIG_CACHE,
  };
}
...
function restoreEnv(prev: EnvSnapshot): void {
  ...
  restoreEnvVar("CLAWX_STATE_DIR", prev.stateDir);
  restoreEnvVar("CLAWX_CONFIG_PATH", prev.configPath);
  restoreEnvVar("CLAWX_SKIP_CHANNELS", prev.skipChannels);
  restoreEnvVar("CLAWX_SKIP_GMAIL_WATCHER", prev.skipGmail);
  restoreEnvVar("CLAWX_SKIP_CRON", prev.skipCron);
  restoreEnvVar("CLAWX_SKIP_CANVAS_HOST", prev.skipCanvas);
  restoreEnvVar("CLAWX_GATEWAY_TOKEN", prev.token);
  restoreEnvVar("CLAWX_GATEWAY_PASSWORD", prev.password);
  restoreEnvVar("CLAWX_DISABLE_CONFIG_CACHE", prev.disableConfigCache);
}
...
async function withOnboardEnv(
  ...
  process.env.CLAWX_DISABLE_CONFIG_CACHE = "1";
  ...
  const configPath = path.join(tempHome, "clawx.json");
  process.env.HOME = tempHome;
  process.env.CLAWX_STATE_DIR = tempHome;
  process.env.CLAWX_CONFIG_PATH = configPath;
  vi.resetModules();
...
describe("onboard (non-interactive): provider auth", () => {
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("clawx-onboard-xai-", async ({ configPath, runtime }) => {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("clawx-onboard-ai-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("clawx-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("clawx-onboard-openai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores Cloudflare AI Gateway API key and metadata", async () => {
    await withOnboardEnv("clawx-onboard-cf-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers Cloudflare auth choice from API key flags", async () => {
    await withOnboardEnv("clawx-onboard-cf-gateway-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
          cloudflareAiGatewayAccountId: "cf-account-id",
