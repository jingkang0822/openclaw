...
const requireAgentDir = () => {
  const agentDir = process.env.CLAWX_AGENT_DIR;
  if (!agentDir) {
    throw new Error("CLAWX_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.CLAWX_STATE_DIR;
  const previousAgentDir = process.env.CLAWX_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.CLAWX_AGENT_DIR;
    } else {
      process.env.CLAWX_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.CLAWX_OAUTH_DIR;
  });

  it("writes auth-profiles.json under CLAWX_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawx-oauth-"));
    process.env.CLAWX_STATE_DIR = tempStateDir;
    process.env.CLAWX_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.CLAWX_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to CLAWX_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawx-minimax-"));
    process.env.CLAWX_STATE_DIR = tempStateDir;
    process.env.CLAWX_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.CLAWX_AGENT_DIR;
