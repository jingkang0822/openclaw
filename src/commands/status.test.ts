...
beforeAll(() => {
  previousProfile = process.env.CLAWX_PROFILE;
  process.env.CLAWX_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.CLAWX_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/clawx-root.js", () => ({
  resolveClawXPackageRoot: vi.fn().mockResolvedValue("/tmp/clawx"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/clawx/pnpm-lock.yaml",
      markerPath: "/tmp/clawx/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("ClawX status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("clawx status --all") ||
          l.includes("clawx --profile isolated status --all") ||
          l.includes("clawx status --all") ||
          l.includes("clawx --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.CLAWX_GATEWAY_TOKEN = "abcd1234";
    try {
