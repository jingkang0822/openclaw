...
export function listAgentEntries(cfg: ClawXConfig): AgentEntry[] {
...
function resolveAgentName(cfg: ClawXConfig, agentId: string) {
...
function resolveAgentModel(cfg: ClawXConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: ClawXConfig): AgentSummary[] {
