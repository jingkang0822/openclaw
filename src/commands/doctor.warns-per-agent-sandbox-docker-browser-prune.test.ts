...
describe("doctor command", () => {
  ...
  it("does not warn when only the active workspace is present", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      ...
      valid: true,
      config: {
        agents: { defaults: { workspace: "/Users/steipete/clawx" } },
      },
      issues: [],
      legacyIssues: [],
    });
    ...
    const legacyPath = path.join("/Users/steipete", "clawx");
    const legacyAgentsPath = path.join(legacyPath, "AGENTS.md");
    const existsSpy = vi.spyOn(fs, "existsSync").mockImplementation((value) => {
      if (value === "/Users/steipete/clawx" || value === legacyPath || value === legacyAgentsPath) {
        return true;
      }
