...
export async function statusCommand(
  ...
  runtime.log(theme.heading("ClawX status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("clawx security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("clawx security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.clawx.ai/faq");
  runtime.log("Troubleshooting: https://docs.clawx.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("clawx status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("clawx logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("clawx status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("clawx gateway probe")}`);
  }
}
