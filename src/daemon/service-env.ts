...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.CLAWX_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel ||
    (process.platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  const stateDir = env.CLAWX_STATE_DIR;
  const configPath = env.CLAWX_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    CLAWX_PROFILE: profile,
    CLAWX_STATE_DIR: stateDir,
    CLAWX_CONFIG_PATH: configPath,
    CLAWX_GATEWAY_PORT: String(port),
    CLAWX_GATEWAY_TOKEN: token,
    CLAWX_LAUNCHD_LABEL: resolvedLaunchdLabel,
    CLAWX_SYSTEMD_UNIT: systemdUnit,
    CLAWX_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    CLAWX_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    CLAWX_STATE_DIR: stateDir,
    CLAWX_CONFIG_PATH: configPath,
    CLAWX_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    CLAWX_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    CLAWX_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    CLAWX_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    CLAWX_LOG_PREFIX: "node",
    CLAWX_SERVICE_MARKER: NODE_SERVICE_MARKER,
    CLAWX_SERVICE_KIND: NODE_SERVICE_KIND,
    CLAWX_SERVICE_VERSION: VERSION,
  };
}
