...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "clawx-docker-setup-"));
...
function createEnv(
  ...
  return {
    ...
    CLAWX_GATEWAY_TOKEN: "test-token",
    CLAWX_CONFIG_DIR: join(sandbox.rootDir, "config"),
    CLAWX_WORKSPACE_DIR: join(sandbox.rootDir, "clawx"),
    ...overrides,
  };
}
...
describe("docker-setup.sh", () => {
  it("handles unset optional env vars under strict mode", async () => {
    ...
    const env = createEnv(sandbox, {
      CLAWX_DOCKER_APT_PACKAGES: undefined,
      CLAWX_EXTRA_MOUNTS: undefined,
      CLAWX_HOME_VOLUME: undefined,
    });
    ...
    expect(envFile).toContain("CLAWX_DOCKER_APT_PACKAGES=");
    expect(envFile).toContain("CLAWX_EXTRA_MOUNTS=");
    expect(envFile).toContain("CLAWX_HOME_VOLUME=");
  });
  ...
  it("supports a home volume when extra mounts are empty", async () => {
    ...
    expect(extraCompose).toContain("clawx-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("clawx-home:");
  });
  ...
  it("plumbs CLAWX_DOCKER_APT_PACKAGES into .env and docker build args", async () => {
    const sandbox = await createDockerSetupSandbox();
    const env = createEnv(sandbox, {
      CLAWX_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      CLAWX_EXTRA_MOUNTS: "",
      CLAWX_HOME_VOLUME: "",
    });
    ...
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("CLAWX_DOCKER_APT_PACKAGES=ffmpeg build-essential");

    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg CLAWX_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
