...
declare const __CLAWX_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __CLAWX_VERSION__ === "string" && __CLAWX_VERSION__) ||
  process.env.CLAWX_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  ...
  const { ensureClawXCliOnPath } = await import("../infra/path-env.js");
  ...
  process.on("uncaughtException", (error) => {
    console.error("[clawx] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
...
void main().catch((err) => {
  console.error("[clawx] Relay failed:", err instanceof Error ? (err.stack ?? err.message) : err);
  process.exit(1);
});
