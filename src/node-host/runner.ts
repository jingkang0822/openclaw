...
import { ensureClawXCliOnPath } from "../infra/path-env.js";
...
const execHostEnforced = process.env.CLAWX_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed = process.env.CLAWX_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureClawXCliOnPath({ pathEnv: process.env.PATH ?? "" });
