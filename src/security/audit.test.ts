...
describe("security audit", () => {
  ...
  it("treats Windows ACL-only perms as secure", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "clawx-security-audit-win-"));
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(stateDir, { recursive: true });
    const configPath = path.join(stateDir, "clawx.json");
  ...
  it("flags Windows ACLs when Users can read the state dir", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "clawx-security-audit-win-open-"));
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(stateDir, { recursive: true });
    const configPath = path.join(stateDir, "clawx.json");
    await fs.writeFile(configPath, "{}\n", "utf-8");
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: ClawXConfig = { session: { dmScope: "main" } };
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    const prevStateDir = process.env.CLAWX_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "clawx-security-audit-discord-"));
    process.env.CLAWX_STATE_DIR = tmp;
    ...
    try {
      ...
      if (prevStateDir == null) {
        ...
        process.env.CLAWX_STATE_DIR = prevStateDir;
      }
    }
  });
  ...
  it("does not flag Discord slash commands when dm.allowFrom includes a Discord snowflake id", async () => {
    ...
    const tmp = await fs.mkdtemp(
      path.join(os.tmpdir(), "clawx-security-audit-discord-allowfrom-snowflake-"),
    );
    process.env.CLAWX_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
  ...
  it("flags Discord slash commands when access-group enforcement is disabled and no users allowlist exists", async () => {
    const prevStateDir = process.env.CLAWX_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "clawx-security-audit-discord-open-"));
    process.env.CLAWX_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: ClawXConfig = {
  ...
  it("flags Slack slash commands without a channel users allowlist", async () => {
    const prevStateDir = process.env.CLAWX_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "clawx-security-audit-slack-"));
    process.env.CLAWX_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: ClawXConfig = {
  ...
  it("flags Slack slash commands when access-group enforcement is disabled", async () => {
    const prevStateDir = process.env.CLAWX_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "clawx-security-audit-slack-open-"));
    process.env.CLAWX_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: ClawXConfig = {
  ...
  it("flags Telegram group commands without a sender allowlist", async () => {
    const prevStateDir = process.env.CLAWX_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "clawx-security-audit-telegram-"));
    process.env.CLAWX_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: ClawXConfig = {
  ...
  it("adds a warning when deep probe fails", async () => {
    const cfg: ClawXConfig = { gateway: { mode: "local" } };
  ...
  it("warns when hooks token reuses the gateway env token", async () => {
    const prevToken = process.env.CLAWX_GATEWAY_TOKEN;
    process.env.CLAWX_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: ClawXConfig = {
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.CLAWX_GATEWAY_TOKEN = prevToken;
      }
    }
  });
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      includeFilesystem: false,
      includeChannelSecurity: false,
      stateDir: "/Users/test/Dropbox/.clawx",
      configPath: "/Users/test/Dropbox/.clawx/clawx.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "clawx-security-audit-"));
    const stateDir = path.join(tmp, "state");
    ...
    try {
      const cfg: ClawXConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "clawx.json"),
      });
  ...
  it("flags plugins with dangerous code patterns (deep audit)", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawx-audit-scanner-"));
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        clawx: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    it("uses remote auth when gateway.mode is remote with URL", async () => {
      ...
      const cfg: ClawXConfig = {
        gateway: {
          ...
          remote: {
            url: "ws://remote.example.com:19789",
    ...
    it("prefers env password over remote password", async () => {
      process.env.CLAWX_GATEWAY_PASSWORD = "env-pass";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: ClawXConfig = {
        gateway: {
          mode: "remote",
