...
import { discoverClawXPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: ClawXPluginDefinition;
  register?: ClawXPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as ClawXPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as ClawXPluginDefinition;
...
export function loadClawXPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverClawXPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "clawx/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: ClawXPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as ClawXPluginModule;
    } catch (err) {
