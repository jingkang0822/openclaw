...
import { loadClawXPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.CLAWX_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `clawx-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "clawx.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.CLAWX_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadClawXPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.CLAWX_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadClawXPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadClawXPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
