...
`;
    ...
    expect(parsed.clawx.emoji).toBe("💾");
    expect(parsed.clawx.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.clawx.emoji).toBe("📝");
    expect(parsed.clawx.events).toEqual(["command"]);
    expect(parsed.clawx.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.clawx.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"clawx": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"clawx": {"events": ["test"]}}');
  });
...
describe("resolveClawXMetadata", () => {
  it("extracts clawx metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveClawXMetadata(frontmatter);
  ...
  it("returns undefined when clawx key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        clawx: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with ClawX" },
            { id: "npm", kind: "npm", package: "@clawx/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@clawx/hook");
  });
    ...
    "clawx":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with ClawX" }],
      },
  }
---
...
`;
    ...
    const clawx = resolveClawXMetadata(frontmatter);
    expect(clawx).toBeDefined();
    expect(clawx?.emoji).toBe("💾");
    expect(clawx?.events).toEqual(["command:new"]);
    expect(clawx?.requires?.config).toEqual(["workspace.dir"]);
    expect(clawx?.install?.[0].kind).toBe("bundled");
  });
