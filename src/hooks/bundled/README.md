...
This directory contains hooks that ship with ClawX. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.clawx.ai/hooks/soul-evil
...
Hooks can be configured in `~/.clawx/clawx.json`:
...
2. Restart gateway: `pkill -9 -f 'clawx.*gateway' && pnpm clawx gateway`
3. Enable the hook: `clawx hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.clawx.ai/hooks
