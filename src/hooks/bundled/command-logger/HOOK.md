...
homepage: https://docs.clawx.ai/hooks#command-logger
metadata:
  {
    "clawx":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.clawx/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.clawx/logs/commands.log ~/.clawx/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/clawx`:
   ```
   /home/username/.clawx/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
