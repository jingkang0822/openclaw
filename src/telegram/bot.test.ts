...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/clawx-telegram-bot-${Math.random().toString(16).slice(2)}.json`,
}));
...
describe("createTelegramBot", () => {
  ...
  it("requests pairing by default for unknown DM senders", async () => {
    ...
    expect(pairingText).toContain("clawx pairing approve telegram PAIRME12");
    ...
    await handler({
      message: {
        ...
        reply_to_message: {
          message_id: 42,
          text: "original reply",
          from: { id: 999, first_name: "ClawX" },
        },
      },
      me: { id: 999, username: "clawx_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  ...
  it("honors routed group activation from session store", async () => {
    ...
    const replySpy = replyModule.__replySpy as unknown as ReturnType<typeof vi.fn>;
    replySpy.mockReset();
    const storeDir = fs.mkdtempSync(path.join(os.tmpdir(), "clawx-telegram-"));
