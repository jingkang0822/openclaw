...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "clawx-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".clawx", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<ClawXConfig> = {},
): ClawXConfig {
  const base: ClawXConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "clawx"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".clawx", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
