...
describe("canvas host state dir defaults", () => {
  ...
  it("uses CLAWX_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "clawx-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("ClawX Canvas");
    } finally {
      await handler.close();
