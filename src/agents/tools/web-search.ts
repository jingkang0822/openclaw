...
type WebSearchConfig = NonNullable<ClawXConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: ClawXConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("clawx configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.clawx.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://clawx.ai",
      "X-Title": "ClawX Web Search",
    },
    body: JSON.stringify({
      model,
      messages: [
