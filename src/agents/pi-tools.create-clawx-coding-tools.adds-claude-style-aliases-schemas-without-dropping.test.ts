...
import { __testing, createClawXCodingTools } from "./pi-tools.js";
...
describe("createClawXCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createClawXCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawx-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "clawx-outside.txt");
