...
export function listChannelSupportedActions(params: {
  ...
  const cfg = params.cfg ?? ({} as ClawXConfig);
...
export function listChannelAgentTools(params: { cfg?: ClawXConfig }): ChannelAgentTool[] {
...
function runPluginListActions(plugin: ChannelPlugin, cfg: ClawXConfig): ChannelMessageActionName[] {
  if (!plugin.actions?.listActions) {
    return [];
  }
  try {
