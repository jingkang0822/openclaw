...
const defaultTools = createClawXCodingTools();

describe("createClawXCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawx-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    ...
    try {
      ...
      const contents = "Hello from clawx read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "clawx-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "clawx-workspace"),
    ...
    const tools = createClawXCodingTools({ sandbox });
