...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "clawx.sandbox=1");
  args.push("--label", `clawx.sessionKey=${params.scopeKey}`);
  args.push("--label", `clawx.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `clawx.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("clawx.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`clawx sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`clawx sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("clawx sandbox recreate --all");
}
