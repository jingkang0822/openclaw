...
import { createClawXCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("clawx-ws-", async (workspaceDir) => {
      await withTempDir("clawx-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createClawXCodingTools({ workspaceDir });
    ...
    await withTempDir("clawx-ws-", async (workspaceDir) => {
      await withTempDir("clawx-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello clawx");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("defaults exec cwd to workspaceDir when workdir is omitted", async () => {
    await withTempDir("clawx-ws-", async (workspaceDir) => {
      const tools = createClawXCodingTools({ workspaceDir, exec: { host: "gateway" } });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("clawx-ws-", async (workspaceDir) => {
      await withTempDir("clawx-override-", async (overrideDir) => {
        const tools = createClawXCodingTools({ workspaceDir, exec: { host: "gateway" } });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("clawx-sandbox-", async (sandboxDir) => {
      await withTempDir("clawx-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
        ...
        const tools = createClawXCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
