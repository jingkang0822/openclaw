...
/**
 * Cleanup suspended ClawX CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: ClawXConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawx-cli-images-"));
  const paths: string[] = [];
