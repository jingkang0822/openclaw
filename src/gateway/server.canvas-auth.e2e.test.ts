...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.CLAWX_CONFIG_PATH;
  const prevDisableCache = process.env.CLAWX_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "clawx-canvas-auth-test-"));
  const configPath = path.join(dir, "clawx.json");
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.CLAWX_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.CLAWX_DISABLE_CONFIG_CACHE;
    } else {
      process.env.CLAWX_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
