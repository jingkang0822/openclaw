...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.CLAWX_SKIP_CHANNELS;
    prevSkipGmail = process.env.CLAWX_SKIP_GMAIL_WATCHER;
    process.env.CLAWX_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.CLAWX_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.CLAWX_SKIP_GMAIL_WATCHER;
    } else {
      process.env.CLAWX_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
