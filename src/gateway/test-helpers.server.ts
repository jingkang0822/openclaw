...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.CLAWX_STATE_DIR;
  previousConfigPath = process.env.CLAWX_CONFIG_PATH;
  previousSkipBrowserControl = process.env.CLAWX_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.CLAWX_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.CLAWX_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.CLAWX_BUNDLED_PLUGINS_DIR;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "clawx-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.CLAWX_STATE_DIR = path.join(tempHome, ".clawx");
  delete process.env.CLAWX_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.CLAWX_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.CLAWX_SKIP_GMAIL_WATCHER = "1";
  process.env.CLAWX_SKIP_CANVAS_HOST = "1";
  process.env.CLAWX_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "clawx-test-no-bundled-extensions")
    : "clawx-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "clawx-test-"))
    : path.join(tempHome, ".clawx-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.CLAWX_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.CLAWX_CONFIG_PATH;
    } else {
      process.env.CLAWX_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.CLAWX_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.CLAWX_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.CLAWX_SKIP_GMAIL_WATCHER;
    } else {
      process.env.CLAWX_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.CLAWX_SKIP_CANVAS_HOST;
    } else {
      process.env.CLAWX_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.CLAWX_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.CLAWX_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.CLAWX_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.CLAWX_GATEWAY_TOKEN = fallbackToken;
  }
