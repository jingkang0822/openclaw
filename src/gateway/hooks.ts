...
export function resolveHooksConfig(cfg: ClawXConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: ClawXConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-clawx-token"] === "string" ? req.headers["x-clawx-token"].trim() : "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
