...
export async function startGatewayServer(
  ...
  process.env.CLAWX_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "CLAWX_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "CLAWX_RAW_STREAM_PATH",
  ...
  if (configSnapshot.legacyIssues.length > 0) {
    ...
    if (!migrated) {
      throw new Error(
        `Legacy config entries detected but auto-migration failed. Run "${formatCliCommand("clawx doctor")}" to migrate.`,
      );
    }
