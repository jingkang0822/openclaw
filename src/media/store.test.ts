...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "CLAWX_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "clawx-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.CLAWX_STATE_DIR = path.join(home, ".clawx");
    ...
    await fs.mkdir(path.join(home, ".clawx"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.clawx${path.sep}media`);
