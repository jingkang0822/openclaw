...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".clawx-profile-decorated");
}
 ...
 */
export function decorateClawXProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_CLAWX_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_CLAWX_BROWSER_COLOR).toUpperCase();
