...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const clawx = resolveProfile(resolved, "clawx");
    expect(clawx?.driver).toBe("clawx");
    expect(clawx?.cdpPort).toBe(18800);
    expect(clawx?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from CLAWX_GATEWAY_PORT when unset", () => {
    const prev = process.env.CLAWX_GATEWAY_PORT;
    process.env.CLAWX_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const clawx = resolveProfile(resolved, "clawx");
      expect(clawx?.cdpPort).toBe(19012);
      expect(clawx?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.CLAWX_GATEWAY_PORT;
      } else {
        process.env.CLAWX_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const clawx = resolveProfile(resolved, "clawx");
      expect(clawx?.cdpPort).toBe(19022);
      expect(clawx?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.CLAWX_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "clawx");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        clawx: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("clawx");
  });
});
