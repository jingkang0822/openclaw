import type { BrowserConfig, BrowserProfileConfig, ClawXConfig } from "../config/config.js";
...
import {
  DEFAULT_CLAWX_BROWSER_COLOR,
  DEFAULT_CLAWX_BROWSER_ENABLED,
  DEFAULT_BROWSER_EVALUATE_ENABLED,
  DEFAULT_BROWSER_DEFAULT_PROFILE_NAME,
  DEFAULT_CLAWX_BROWSER_PROFILE_NAME,
} from "./constants.js";
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "clawx" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_CLAWX_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "clawx" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_CLAWX_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_CLAWX_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an ClawX browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for clawx/clawx CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: ClawXConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_CLAWX_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "clawx";
