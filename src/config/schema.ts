...
import { ClawXSchema } from "./zod-schema.js";
...
export type ConfigSchema = ReturnType<typeof ClawXSchema.toJSONSchema>;
...
const FIELD_HELP: Record<string, string> = {
  "meta.lastTouchedVersion": "Auto-set when ClawX writes the config.",
  ...
  "gateway.controlUi.basePath": "Optional URL prefix where the Control UI is served (e.g. /clawx).",
  ...
  "diagnostics.cacheTrace.filePath":
    "JSONL output path for cache trace logs (default: $CLAWX_STATE_DIR/logs/cache-trace.jsonl).",
  ...
  "agents.defaults.memorySearch.store.path":
    "SQLite index path (default: ~/.clawx/memory/{agentId}.sqlite).",
  ...
  "memory.backend": 'Memory backend ("builtin" for ClawX embeddings, "qmd" for QMD sidecar).',
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `clawx plugins update` to locate install sources).",
  "plugins.installs.*.source": 'Install source ("npm", "archive", or "path").',
  "plugins.installs.*.spec": "Original npm spec used for install (if source is npm).",
  "plugins.installs.*.sourcePath": "Original archive/path used for install (if any).",
  "plugins.installs.*.installPath":
    "Resolved install directory (usually ~/.clawx/extensions/<id>).",
...
const FIELD_PLACEHOLDERS: Record<string, string> = {
  ...
  "agents.list[].identity.avatar": "avatars/clawx.png",
};
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const schema = ClawXSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "ClawXConfig";
