import type { ClawXConfig } from "./config.js";
...
function resolveChannelConfig(cfg: ClawXConfig, channelId: string): Record<string, unknown> | null {
  const channels = cfg.channels as Record<string, unknown> | undefined;
  const entry = channels?.[channelId];
  return isRecord(entry) ? entry : null;
}

function isTelegramConfigured(cfg: ClawXConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: ClawXConfig, env: NodeJS.ProcessEnv): boolean {
...
function isIrcConfigured(cfg: ClawXConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: ClawXConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: ClawXConfig): boolean {
...
function isIMessageConfigured(cfg: ClawXConfig): boolean {
...
function isWhatsAppConfigured(cfg: ClawXConfig): boolean {
...
function isGenericChannelConfigured(cfg: ClawXConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: ClawXConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: ClawXConfig, providerId: string): boolean {
...
function resolveConfiguredPlugins(cfg: ClawXConfig, env: NodeJS.ProcessEnv): PluginEnableChange[] {
...
function isPluginExplicitlyDisabled(cfg: ClawXConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: ClawXConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: ClawXConfig, pluginId: string): ClawXConfig {
...
function registerPluginEntry(cfg: ClawXConfig, pluginId: string): ClawXConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
