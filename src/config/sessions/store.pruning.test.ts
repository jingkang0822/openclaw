...
// Mock loadConfig so resolveMaintenanceConfig() never reads a real clawx.json.
...
describe("rotateSessionFile", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawx-rotate-"));
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawx-pruning-integ-"));
    storePath = path.join(testDir, "sessions.json");
    savedCacheTtl = process.env.CLAWX_SESSION_CACHE_TTL_MS;
    process.env.CLAWX_SESSION_CACHE_TTL_MS = "0";
    clearSessionStoreCacheForTest();
  ...
  afterEach(async () => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.CLAWX_SESSION_CACHE_TTL_MS;
    } else {
      process.env.CLAWX_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
