...
describe("config env vars", () => {
  ...
  it("loads ${VAR} substitutions from ~/.clawx/.env on repeated runtime loads", async () => {
    await withTempHome(async (home) => {
      await withEnvOverride(
        {
          CLAWX_STATE_DIR: path.join(home, ".clawx"),
          CLAWDBOT_STATE_DIR: undefined,
          CLAWX_HOME: undefined,
          CLAWDBOT_HOME: undefined,
          BRAVE_API_KEY: undefined,
          CLAWX_DISABLE_CONFIG_CACHE: "1",
        },
        async () => {
