...
import type { ClawXConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
import { compareClawXVersions } from "./version.js";
...
function coerceConfig(value: unknown): ClawXConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as ClawXConfig;
}
...
function stampConfigVersion(cfg: ClawXConfig): ClawXConfig {
...
function warnIfConfigFromFuture(cfg: ClawXConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareClawXVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer ClawX (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: ClawXConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): ClawXConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as ClawXConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as ClawXConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: ClawXConfig) {
    clearConfigCache();
...
// module scope. `CLAWX_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.CLAWX_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.CLAWX_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): ClawXConfig {
...
export async function writeConfigFile(cfg: ClawXConfig): Promise<void> {
  clearConfigCache();
