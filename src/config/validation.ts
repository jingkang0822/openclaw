...
import type { ClawXConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: ClawXConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: ClawXConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = ClawXSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as ClawXConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as ClawXConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as ClawXConfig)),
    ),
  };
}
