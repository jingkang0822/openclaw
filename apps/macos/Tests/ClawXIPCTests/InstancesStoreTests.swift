...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: ClawXProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: ClawXProtocol.AnyCodable] = [
            "presence": .init([ClawXProtocol.AnyCodable(entry)]),
        ]
        let payload = ClawXProtocol.AnyCodable(payloadMap)
