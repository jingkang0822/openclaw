...
@Suite struct GatewayEndpointStoreTests {
    ...
    @Test func resolveGatewayTokenPrefersEnvAndFallsBackToLaunchd() {
        let snapshot = LaunchAgentPlistSnapshot(
            programArguments: [],
            environment: ["CLAWX_GATEWAY_TOKEN": "launchd-token"],
            stdoutPath: nil,
            stderrPath: nil,
            port: nil,
            bind: nil,
        ...
        let envToken = GatewayEndpointStore._testResolveGatewayToken(
            isRemote: false,
            root: [:],
            env: ["CLAWX_GATEWAY_TOKEN": "env-token"],
    ...
    @Test func resolveGatewayPasswordFallsBackToLaunchd() {
        let snapshot = LaunchAgentPlistSnapshot(
            programArguments: [],
            environment: ["CLAWX_GATEWAY_PASSWORD": "launchd-pass"],
            stdoutPath: nil,
            stderrPath: nil,
            port: nil,
            bind: nil,
    ...
    @Test func normalizeGatewayUrlAddsDefaultPortForWs() {
        ...
        #expect(url?.port == 19789)
        #expect(url?.absoluteString == "ws://gateway:19789")
    }
}
