...
struct ClawXMacCLI {
    static func main() async {
        ...
        default:
            fputs("clawx-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      clawx-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      clawx-mac discover [--timeout <ms>] [--json] [--include-local]
      clawx-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      clawx-mac connect
      clawx-mac connect --url ws://127.0.0.1:19789 --json
      clawx-mac discover --timeout 3000 --json
      clawx-mac wizard --mode local
    """)
}
