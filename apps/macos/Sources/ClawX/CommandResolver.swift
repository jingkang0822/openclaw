...
enum CommandResolver {
    private static let projectRootDefaultsKey = "clawx.gatewayProjectRootPath"
    private static let helperName = "clawx"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let clawxEntry = root.appendingPathComponent("clawx.mjs").path
        if FileManager().isReadableFile(atPath: clawxEntry) { return clawxEntry }
        let binEntry = root.appendingPathComponent("bin/clawx.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/clawx")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let clawxPaths = self.clawxManagedPaths(home: home)
        if !clawxPaths.isEmpty {
            extras.insert(contentsOf: clawxPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + clawxPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func clawxManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".clawx"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func clawxExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectClawXExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("clawx.mjs").path,
            root.appendingPathComponent("bin/clawx.js").path,
        ]
    ...
    static func hasAnyClawXInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.clawxExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let clawxPath = self.projectClawXExecutable(projectRoot: root) {
                return [clawxPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "clawx", subcommand] + extraArgs
            }
            if let clawxPath = self.clawxExecutable(searchPaths: searchPaths) {
                return [clawxPath, subcommand] + extraArgs
            }

            let missingEntry = """
            clawx entrypoint missing (looked for dist/index.js or clawx.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real clawx CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/clawx"
        ...
        \(cliSection)
        \(projectSection)
        if command -v clawx >/dev/null 2>&1; then
          CLI="$(command -v clawx)"
          clawx \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/clawx.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/clawx.mjs"
            node "$PRJ/clawx.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/clawx.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/clawx.js"
            node "$PRJ/bin/clawx.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent clawx"
          pnpm --silent clawx \(quotedArgs);
        else
          echo "clawx CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? ClawXConfigFile.loadDict()
