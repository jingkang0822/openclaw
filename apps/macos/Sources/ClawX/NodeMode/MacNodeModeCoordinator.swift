...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.clawx", category: "mac-node")
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: ClawXNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [ClawXCapability.canvas.rawValue, ClawXCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(ClawXCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if ClawXLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(ClawXCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        ...
        if capsSet.contains(ClawXCapability.camera.rawValue) {
        ...
        if capsSet.contains(ClawXCapability.location.rawValue) {
