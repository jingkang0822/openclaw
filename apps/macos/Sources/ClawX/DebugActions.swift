...
enum DebugActions {
    private static let verboseDefaultsKey = "clawx.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "clawx.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = ClawXPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "ClawX", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = ClawXPaths.configURL
        guard
