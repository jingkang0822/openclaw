...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (ClawX token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: ClawXOAuthStore.AnthropicOAuthStatus = ClawXOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.clawx", category: "anthropic-oauth")
...
enum ClawXOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let clawxOAuthDirEnv = "CLAWX_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "ClawX OAuth token file not found"
            case .unreadableFile: "ClawX OAuth token file not readable"
            case .invalidJSON: "ClawX OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in ClawX OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "ClawX OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.clawxOAuthDirEnv]?
        ...
        let preferred = home.appendingPathComponent(".clawx", isDirectory: true)
