...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["CLAWX_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/clawx")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("clawx-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("clawx-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/clawx/ (rolling or stdout), or nil if none exist.
