...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: ClawXProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.clawx", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: ClawXKit.AnyCodable]()) {
                $0[$1.key] = ClawXKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: ClawXProtocol.AnyCodable?) -> [String: ClawXProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: ClawXProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: ClawXKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: ClawXProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("clawx.control.heartbeat")
    static let controlAgentEvent = Notification.Name("clawx.control.agent")
}
