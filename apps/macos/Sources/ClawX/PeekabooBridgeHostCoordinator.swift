...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.clawx", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: ClawXPeekabooBridgeServices?
    private static var clawxSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("ClawX", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = ClawXPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.clawxSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.clawxSocketPath, privacy: .public)")
    }
...
@MainActor
private final class ClawXPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.clawx.peekaboo")
