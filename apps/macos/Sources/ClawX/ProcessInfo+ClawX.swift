...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.clawx.mac`) even if the shipped
    ...
    static func resolveNixMode(
        ...
        if environment["CLAWX_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "clawx.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "clawx.nixMode") { return true }
