...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.clawx", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> ClawXSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: ClawXSessionPreviewEntry,
    ...
    private static func cache(payload: ClawXSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: ClawXChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [ClawXKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [ClawXKit.AnyCodable]) -> [ClawXChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(ClawXChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: ClawXChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: ClawXChatMessage) -> Bool {
    ...
    private static func toolNames(for message: ClawXChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: ClawXChatMessage) -> String? {
