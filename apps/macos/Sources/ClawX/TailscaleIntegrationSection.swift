...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.clawx/clawx.json. Prefer CLAWX_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.clawx/clawx.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.clawx/clawx.json. Restart the gateway to apply."
        }
