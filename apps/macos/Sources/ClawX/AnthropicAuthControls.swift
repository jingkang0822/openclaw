...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: ClawXOAuthStore.AnthropicOAuthStatus = ClawXOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: ClawXOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = ClawXOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = ClawXOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try ClawXOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. ClawX can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: ClawXOAuthStore.AnthropicOAuthStatus,
