...
private let webChatSwiftLogger = Logger(subsystem: "ai.clawx", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: ClawXChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> ClawXChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(ClawXChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> ClawXChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                ClawXGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<ClawXChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any ClawXChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = ClawXChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: ClawXChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<ClawXChatView>) -> NSViewController
    {
