...
final class ContactsService: ContactsServicing {
    ...
    func search(params: ClawXContactsSearchParams) async throws -> ClawXContactsSearchPayload {
        ...
        return ClawXContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: ClawXContactsAddParams) async throws -> ClawXContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return ClawXContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return ClawXContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> ClawXContactPayload {
        ClawXContactPayload(
