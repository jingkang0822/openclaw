...
enum GatewaySettingsStore {
    private static let gatewayService = "ai.clawx.gateway"
    private static let nodeService = "ai.clawx.node"
...
enum GatewayDiagnostics {
    private static let logger = Logger(subsystem: "ai.clawx.ios", category: "GatewayDiag")
    private static let queue = DispatchQueue(label: "ai.clawx.gateway.diagnostics")
    ...
    private static var fileURL: URL? {
        FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first?
            .appendingPathComponent("clawx-gateway.log")
    }
