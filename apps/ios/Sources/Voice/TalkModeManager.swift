...
final class TalkModeManager: NSObject {
    ...
    private var pttCompletion: CheckedContinuation<ClawXTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = ClawXTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> ClawXTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return ClawXTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> ClawXTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> ClawXTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return ClawXTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> ClawXTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = ClawXTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: ClawXTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: ClawXAgentEventPayload.self) else {
                continue
            }
