...
final class MotionService: MotionServicing {
    func activities(params: ClawXMotionActivityParams) async throws -> ClawXMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[ClawXMotionActivityEntry], Error>) in
        ...
        return ClawXMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: ClawXPedometerParams) async throws -> ClawXPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<ClawXPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = ClawXPedometerPayload(
