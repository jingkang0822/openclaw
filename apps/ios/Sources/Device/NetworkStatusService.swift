...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> ClawXNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> ClawXNetworkStatusPayload {
        let status: ClawXNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> ClawXNetworkStatusPayload {
        ClawXNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
