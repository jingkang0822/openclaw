...
struct IOSGatewayChatTransport: ClawXChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> ClawXChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(ClawXChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> ClawXChatHistoryPayload {
        ...
        return try JSONDecoder().decode(ClawXChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [ClawXChatAttachmentPayload]) async throws -> ClawXChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [ClawXChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(ClawXChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(ClawXGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ClawXGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ClawXChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ClawXAgentEventPayload.self)
                        {
