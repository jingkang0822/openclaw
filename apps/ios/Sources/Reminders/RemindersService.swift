...
final class RemindersService: RemindersServicing {
    func list(params: ClawXRemindersListParams) async throws -> ClawXRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[ClawXReminderPayload], Error>) in
        ...
        return ClawXRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: ClawXRemindersAddParams) async throws -> ClawXRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = ClawXReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return ClawXRemindersAddPayload(reminder: payload)
    }
