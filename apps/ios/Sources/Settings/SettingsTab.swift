...
struct SettingsTab: View {
    ...
    @AppStorage("location.enabledMode") private var locationEnabledModeRaw: String = ClawXLocationMode.off.rawValue
    ...
    @AppStorage("gateway.manual.port") private var manualGatewayPort: Int = 19789
    ...
    @State private var lastLocationModeRaw: String = ClawXLocationMode.off.rawValue
    ...
    private let gatewayLogger = Logger(subsystem: "ai.clawx.ios", category: "GatewaySettings")
    ...
    var body: some View {
        NavigationStack {
                    ...
                    DisclosureGroup(isExpanded: self.$gatewayExpanded) {
                        ...
                        Text(
                            "Use this when mDNS/Bonjour discovery is blocked. "
                                + "Leave port empty for 443 on tailnet DNS (TLS) or 19789 otherwise.")
                            .font(.footnote)
                            .foregroundStyle(.secondary)
                ...
                Section("Device") {
                    DisclosureGroup("Features") {
                        ...
                        Picker("Location Access", selection: self.$locationEnabledModeRaw) {
                            Text("Off").tag(ClawXLocationMode.off.rawValue)
                            Text("While Using").tag(ClawXLocationMode.whileUsing.rawValue)
                            Text("Always").tag(ClawXLocationMode.always.rawValue)
                        }
                        ...
                        Text("Keeps the screen awake while ClawX is open.")
                            .font(.footnote)
                            .foregroundStyle(.secondary)
                    }
            ...
            .onChange(of: self.locationEnabledModeRaw) { _, newValue in
                ...
                guard let mode = ClawXLocationMode(rawValue: newValue) else { return }
                Task {
    ...
    private var locationMode: ClawXLocationMode {
        ClawXLocationMode(rawValue: self.locationEnabledModeRaw) ?? .off
    }
