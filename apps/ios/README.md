...
This is an **alpha** iOS app that connects to an ClawX Gateway as a `role: node`.
...
3. On iOS: ClawX → Settings → Gateway → paste setup code → Connect
...
cd apps/ios
xcodegen generate
xcodebuild test -project ClawX.xcodeproj -scheme ClawX -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/ClawXKit` contains the shared transport/types used by the iOS app.
