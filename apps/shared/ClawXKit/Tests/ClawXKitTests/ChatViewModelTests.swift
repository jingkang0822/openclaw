...
private final class TestChatTransport: @unchecked Sendable, ClawXChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [ClawXChatHistoryPayload]
    private let sessionsResponses: [ClawXChatSessionsListResponse]

    private let stream: AsyncStream<ClawXChatTransportEvent>
    private let continuation: AsyncStream<ClawXChatTransportEvent>.Continuation

    init(
        historyResponses: [ClawXChatHistoryPayload],
        sessionsResponses: [ClawXChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<ClawXChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> ClawXChatHistoryPayload {
        ...
        return self.historyResponses.last ?? ClawXChatHistoryPayload(
        ...
        attachments _: [ClawXChatAttachmentPayload]) async throws -> ClawXChatSendResponse
    {
        ...
        return ClawXChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> ClawXChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? ClawXChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: ClawXChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = ClawXChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = ClawXChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { ClawXChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = ClawXChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = ClawXChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = ClawXChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { ClawXChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
