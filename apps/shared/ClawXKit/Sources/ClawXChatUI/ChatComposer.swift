...
@MainActor
struct ClawXChatComposer: View {
    @Bindable var viewModel: ClawXChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(ClawXChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(ClawXChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \ClawXPendingAttachment.id)
                { (att: ClawXPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            ClawXPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(ClawXChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(ClawXChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(ClawXChatTheme.subtleCard)
        .clipShape(Capsule())
    }
