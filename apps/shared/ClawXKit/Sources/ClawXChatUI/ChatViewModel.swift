...
private let chatUILogger = Logger(subsystem: "ai.clawx", category: "ClawXChatUI")

@MainActor
@Observable
public final class ClawXChatViewModel {
    public private(set) var messages: [ClawXChatMessage] = []
    ...
    public var attachments: [ClawXPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [ClawXChatPendingToolCall] = []
    public private(set) var sessions: [ClawXChatSessionEntry] = []
    private let transport: any ClawXChatTransport
    ...
    private var pendingToolCallsById: [String: ClawXChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any ClawXChatTransport) {
    ...
    public var sessionChoices: [ClawXChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [ClawXChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [ClawXChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: ClawXPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [ClawXChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: ClawXChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [ClawXChatMessage]) -> [ClawXChatMessage] {
        var result: [ClawXChatMessage] = []
    ...
    private static func dedupeKey(for message: ClawXChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [ClawXChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> ClawXChatAttachmentPayload in
            ClawXChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> ClawXChatSessionEntry {
        ClawXChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: ClawXChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: ClawXChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: ClawXAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = ClawXChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> ClawXPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
