package ai.clawx.android.voice
...
class VoiceWakeCommandExtractorTest {
  ...
  fun extractsCommandAfterTriggerWord() {
    val res = VoiceWakeCommandExtractor.extractCommand("Claude take a photo", listOf("clawx", "claude"))
    assertEquals("take a photo", res)
  }

  @Test
  fun extractsCommandWithPunctuation() {
    val res = VoiceWakeCommandExtractor.extractCommand("hey clawx, what's the weather?", listOf("clawx"))
