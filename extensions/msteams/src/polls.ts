...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["clawxPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["clawx", "pollId"]) ??
    readNestedString(value, ["clawx", "poll", "id"]) ??
    readNestedString(value, ["data", "clawxPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "clawx", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "clawx poll vote",
          displayText: "Vote recorded",
          value: { clawxPollId: pollId, pollId },
        },
      },
    ],
  };
