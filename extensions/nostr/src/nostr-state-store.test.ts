import type { PluginRuntime } from "clawx/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.CLAWX_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "clawx-nostr-"));
  process.env.CLAWX_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        ...
        return path.join(homedir(), ".clawx");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.CLAWX_STATE_DIR = previous;
    }
