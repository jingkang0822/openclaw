...
ClawX extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
clawx channels login --channel zalouser
...
```bash
clawx message send --channel zalouser --target <threadId> --message "Hello from ClawX!"
```
...
```bash
clawx channels login --channel zalouser              # Login via QR
clawx channels login --channel zalouser --account work
clawx channels status --probe
clawx channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
clawx directory self --channel zalouser
clawx directory peers list --channel zalouser --query "name"
clawx directory groups list --channel zalouser --query "work"
clawx directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
clawx message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
clawx message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `clawx channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
clawx channels login --channel zalouser --account work
clawx message send --channel zalouser --account work --target <id> --message "Hello"
