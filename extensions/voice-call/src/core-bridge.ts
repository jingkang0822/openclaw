...
function resolveClawXRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.CLAWX_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set CLAWX_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveClawXRoot(), "dist", "extensionAPI.js");
