...
// NOTE: This extension is intended to be bundled with ClawX.
// When running from source (tests/dev), ClawX internals live under src/.
...
export function createLlmTaskTool(api: ClawXPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via clawx.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawx-llm-task-"));
