...
`clawx.invoke --each`) without adding new ClawX code per workflow.
...
This extension depends on ClawX internal modules (the embedded agent runner).
It is intended to ship as a **bundled** ClawX extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.clawx/extensions` as a standalone plugin directory.
