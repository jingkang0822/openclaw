 ...
 */
function setTwitchAccount(cfg: ClawXConfig, account: Partial<TwitchAccountConfig>): ClawXConfig {
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: CLAWX_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: ClawXConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var CLAWX_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.CLAWX_TWITCH_ACCESS_TOKEN?.trim();
