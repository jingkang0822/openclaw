/**
 * Twitch channel plugin for ClawX.
 *
...
import { buildChannelConfigSchema } from "clawx/plugin-sdk";
import type {
 ...
 * for ClawX. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: ClawXConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: ClawXConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: ClawXConfig): boolean => {
