...
function resolveGatewayPort(cfg: ClawXPluginApi["config"]): number {
  const envRaw =
    process.env.CLAWX_GATEWAY_PORT?.trim() || process.env.CLAWDBOT_GATEWAY_PORT?.trim();
  if (envRaw) {
...
async function resolveTailnetHost(api: ClawXPluginApi): Promise<string | null> {
...
function resolveAuth(cfg: ClawXPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: ClawXPluginApi): Promise<ResolveUrlResult> {
