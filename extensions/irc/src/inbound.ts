...
export async function handleIrcInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as ClawXConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as ClawXConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as ClawXConfig);
