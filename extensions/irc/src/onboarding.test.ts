import type { RuntimeEnv, WizardPrompter } from "clawx/plugin-sdk";
...
describe("irc onboarding", () => {
  it("configures host and nick via onboarding prompts", async () => {
    ...
    expect(result.cfg.channels?.irc?.nick).toBe("clawx-bot");
    expect(result.cfg.channels?.irc?.tls).toBe(true);
    expect(result.cfg.channels?.irc?.channels).toEqual(["#clawx", "#ops"]);
    expect(result.cfg.channels?.irc?.groupPolicy).toBe("allowlist");
    expect(Object.keys(result.cfg.channels?.irc?.groups ?? {})).toEqual(["#clawx", "#ops"]);
  });
