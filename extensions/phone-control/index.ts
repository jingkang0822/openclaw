import type { ClawXPluginApi, ClawXPluginService } from "clawx/plugin-sdk";
...
function normalizeDenyList(cfg: ClawXPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: ClawXPluginApi["config"]): string[] {
...
export default function register(api: ClawXPluginApi) {
  ...
  const timerService: ClawXPluginService = {
