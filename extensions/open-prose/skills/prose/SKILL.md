...
metadata: { "clawx": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == ClawX `sessions_spawn`
- **File I/O** == ClawX `read`/`write`
- **Remote fetch** == ClawX `web_fetch` (or `exec` with curl when POST is required)
