...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("CLAWX_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.CLAWX_STATE_DIR = path.join(base, ".clawx");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "clawx-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".clawx", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
