...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `clawx-e2e-${name}-`));
  const configDir = path.join(homeDir, ".clawx");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          CLAWX_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        CLAWX_GATEWAY_TOKEN: inst.gatewayToken,
        CLAWX_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          CLAWX_GATEWAY_PORT: String(gwA.port),
          CLAWX_GATEWAY_TOKEN: gwA.gatewayToken,
          CLAWX_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          CLAWX_GATEWAY_PORT: String(gwB.port),
          CLAWX_GATEWAY_TOKEN: gwB.gatewayToken,
          CLAWX_GATEWAY_PASSWORD: "",
        }),
