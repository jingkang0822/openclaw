...
This document describes how the ClawX ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`clawx acp` exposes an ACP agent over stdio and forwards prompts to a running
ClawX Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a ClawX Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `clawx acp` over stdio.

Example config:

```bash
clawx config set gateway.remote.url wss://gateway-host:19789
clawx config set gateway.remote.token <token>
```

Example run:

```bash
clawx acp --url wss://gateway-host:19789 --token <token>
```
...
```bash
clawx acp --session agent:main:main
clawx acp --session agent:design:main
clawx acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “ClawX ACP” to start a thread.

## Execution Model

- ACP client spawns `clawx acp` and speaks ACP messages over stdio.
...
```bash
clawx acp --session agent:main:main
clawx acp --session-label "support inbox"
clawx acp --reset-session
```
...
`clawx acp` resolves the Gateway URL and auth from CLI flags or config:
