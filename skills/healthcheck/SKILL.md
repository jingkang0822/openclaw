---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for ClawX deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, ClawX cron scheduling for periodic checks, or version status checks on a machine running ClawX (laptop, workstation, Pi, VPS).
---

# ClawX Host Hardening

## Overview

Assess and harden the host running ClawX, then align it to a user-defined risk tolerance without breaking access. Use ClawX security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim ClawX changes the host firewall, SSH, or OS updates; it does not.
...
5. ClawX gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if ClawX can access sensitive data.
...
### 2) Run ClawX security audits (read-only)

As part of the default read-only checks, run `clawx security audit --deep`. Only offer alternatives if the user requests them:

1. `clawx security audit` (faster, non-probing)
2. `clawx security audit --json` (structured output)

Offer to apply ClawX safe defaults (numbered):

1. `clawx security audit --fix`

Be explicit that `--fix` only tightens ClawX defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check ClawX version/update status (read-only)

As part of the default read-only checks, run `clawx update status`.
...
- Credential hygiene notes (location of ClawX creds, prefer disk encryption)
...
After ClawX install or first hardening pass, run at least one baseline audit and version check:

- `clawx security audit`
- `clawx security audit --deep`
- `clawx update status`

Ongoing monitoring is recommended. Use the ClawX cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `clawx cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `clawx update status`
...
Before creating, `clawx cron list` and match on exact `name`. If found, `clawx cron edit <id> ...`.
If not found, `clawx cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `clawx update status` (preferred for source checkouts and channels)
2. `npm view clawx version` (published npm version)

## ClawX command accuracy

Use only supported commands and flags:

- `clawx security audit [--deep] [--fix] [--json]`
- `clawx status` / `clawx status --deep`
- `clawx health --json`
- `clawx update status`
- `clawx cron add|list|runs|run`

Do not invent CLI flags or imply ClawX enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by ClawX compaction:
