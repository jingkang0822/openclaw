...
metadata:
  { "clawx": { "emoji": "🧵", "os": ["darwin", "linux"], "requires": { "bins": ["tmux"] } } }
---
...
```bash
SOCKET_DIR="${CLAWX_TMUX_SOCKET_DIR:-${CLAWX_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/clawx-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/clawx.sock"
SESSION=clawx-python
...
- Use `CLAWX_TMUX_SOCKET_DIR` (legacy `CLAWX_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$CLAWX_TMUX_SOCKET_DIR/clawx.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `CLAWX_TMUX_SOCKET_DIR`).
