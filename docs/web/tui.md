...
```bash
clawx tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `clawx tui --deliver`
...
- Check the Gateway logs: `clawx logs --follow`.
- Confirm the agent can run: `clawx status` and `clawx models status`.
...
- No agents in picker: check `clawx agents list` and your routing config.
