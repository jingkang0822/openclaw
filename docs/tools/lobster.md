---
title: Lobster
summary: "Typed workflow runtime for ClawX with resumable approval gates."
description: Typed workflow runtime for ClawX — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets ClawX run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: ClawX runs one Lobster tool call and gets a structured result.
...
ClawX launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | clawx.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
clawx.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In ClawX tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the ClawX Gateway (see the [Lobster repo](https://github.com/clawx/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), ClawX keeps core tools enabled. To restrict core
...
→ clawx calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ clawx calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls ClawX tools that do.
