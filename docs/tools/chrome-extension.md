---
summary: "Chrome extension: let ClawX drive your existing Chrome tab"
read_when:
...
The ClawX Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate clawx-managed Chrome profile.
...
ClawX then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the ClawX release (npm package) as static files. There is no separate “build” step.

After upgrading ClawX:

- Re-run `clawx browser extension install` to refresh the installed files under your ClawX state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

ClawX ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want ClawX to control.
...
- `ON`: attached; ClawX can drive that tab.
...
Debugging: `clawx sandbox explain`
...
`clawx browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `clawx browser extension install` first to copy the extension to a stable location under your ClawX state directory.
...
- **This is not isolated** like the dedicated clawx-managed profile.
