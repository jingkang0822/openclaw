...
- **gateway host** → `clawx` process on the gateway machine
...
`~/.clawx/exec-approvals.json` directly.

CLI: `clawx approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
