...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `clawx sandbox explain`.
...
Legacy `agent.*` configs are migrated by `clawx doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=clawx-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${CLAWX_STATE_DIR:-$HOME/.clawx}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
