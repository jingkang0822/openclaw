...
```bash
clawx plugins install @clawx/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
clawx plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "ClawX" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `clawx pairing list nextcloud-talk`
  - `clawx pairing approve nextcloud-talk <CODE>`
