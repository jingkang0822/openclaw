...
2. Create a **Service Account**:
   ...
   - Name it whatever you want (e.g., `clawx-chat`).
...
4. Store the downloaded JSON file on your gateway host (e.g., `~/.clawx/googlechat-service-account.json`).
5. Create a Google Chat app in the [Google Cloud Console Chat Configuration](https://console.cloud.google.com/apis/api/chat.googleapis.com/hangouts-chat):
   - Fill in the **Application info**:
     - **App name**: (e.g. `ClawX`)
     - **Avatar URL**: (e.g. `https://clawx.ai/logo.png`)
     - **Description**: (e.g. `Personal AI Assistant`)
   - Enable **Interactive features**.
   - Under **Functionality**, check **Join spaces and group conversations**.
   - Under **Connection settings**, select **HTTP endpoint URL**.
   - Under **Triggers**, select **Use a common HTTP endpoint URL for all triggers** and set it to your gateway's public URL followed by `/googlechat`.
     - _Tip: Run `clawx status` to find your gateway's public URL._
...
6. **Enable the app status**:
   ...
   - Click **Save** again.
7. Configure ClawX with the service account path + webhook audience:
...
Google Chat webhooks require a public HTTPS endpoint. For security, **only expose the `/googlechat` path** to the internet. Keep the ClawX dashboard and other sensitive endpoints on your private network.
...
With this config, any request to `your-domain.com/` will be ignored or returned as 404, while `your-domain.com/googlechat` is safely routed to ClawX.

### Option C: Cloudflare Tunnel

Configure your tunnel's ingress rules to only route the webhook path:

- **Path**: `/googlechat` -> `http://localhost:19789/googlechat`
- **Default Rule**: HTTP 404 (Not Found)

## How it works

1. Google Chat sends webhook POSTs to the gateway. Each request includes an `Authorization: Bearer <token>` header.
2. ClawX verifies the token against the configured `audienceType` + `audience`:
...
- Check `clawx channels status --probe` for auth errors or missing audience config.
- If no messages arrive, confirm the Chat app's webhook URL + event subscriptions.
- If mention gating blocks replies, set `botUser` to the app's user resource name and verify `requireMention`.
- Use `clawx logs --follow` while sending a test message to see if requests reach the gateway.
