...
Tlon is a decentralized messenger built on Urbit. ClawX connects to your Urbit ship and can
...
```bash
clawx plugins install @clawx/tlon
```

Local checkout (when running from a git repo):

```bash
clawx plugins install ./extensions/tlon
```
...
Use these with `clawx message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, ClawX replies in-thread.
