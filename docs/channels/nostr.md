...
read_when:
  - You want ClawX to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables ClawX to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`clawx onboard`) and `clawx channels add` list optional channel plugins.
...
```bash
clawx plugins install @clawx/nostr
```

Use a local checkout (dev workflows):

```bash
clawx plugins install --link <path-to-clawx>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "clawx@example.com",
        "lud16": "clawx@example.com"
      }
    }
  }
}
