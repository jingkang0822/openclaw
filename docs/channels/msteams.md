...
```bash
clawx plugins install @clawx/msteams
```

Local checkout (when running from a git repo):

```bash
clawx plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
ClawX will offer the local install path automatically.
...
3. Configure ClawX with those credentials.
...
- Talk to ClawX via Teams DMs, group chats, or channels.
...
- On startup, ClawX resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.clawx/clawx.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring ClawX, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                 |
   | ------------------ | ----------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `clawx-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `clawx plugins install @clawx/msteams`
   - From a local checkout: `clawx plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "ClawX in Teams", "full": "ClawX in Teams" },
...
ClawX handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, ClawX only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/ClawXShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

ClawX sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `clawx message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.clawx/msteams-polls.json`.
...
```bash
clawx message send --channel msteams \
...
clawx message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
clawx message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
clawx message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
clawx message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
