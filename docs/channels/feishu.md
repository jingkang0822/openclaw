...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects ClawX to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
clawx plugins install @clawx/feishu
```

Local checkout (when running from a git repo):

```bash
clawx plugins install ./extensions/feishu
```
...
If you just installed ClawX, run the wizard:
...
2. Configuring app credentials in ClawX
...
1. You already ran `clawx channels add` for Feishu
2. The gateway is running (`clawx gateway status`)
...
```bash
clawx pairing approve feishu <CODE>
```
...
2. Run `clawx logs --follow` and look for `chat_id`
...
2. Run `clawx logs --follow` and look for `open_id`
...
| Command                 | Description                   |
| ----------------------- | ----------------------------- |
| `clawx gateway status`  | Show gateway status           |
| `clawx gateway install` | Install/start gateway service |
| `clawx gateway stop`    | Stop gateway service          |
| `clawx gateway restart` | Restart gateway service       |
| `clawx logs --follow`   | Tail gateway logs             |
...
4. Check logs: `clawx logs --follow`
...
5. Ensure the gateway is running: `clawx gateway status`
6. Check logs: `clawx logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.clawx/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.clawx/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
