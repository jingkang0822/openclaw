...
- Talk to ClawX via Discord DMs or guild channels.
...
3. Configure ClawX with `channels.discord.token` (or `DISCORD_BOT_TOKEN` as a fallback).
...
7. Direct chats: secure by default via `channels.discord.dm.policy` (default: `"pairing"`). Unknown senders get a pairing code (expires after 1 hour); approve via `clawx pairing approve discord <code>`.
...
This is the “Discord Developer Portal” setup for running ClawX in a server (guild) channel like `#help`.
...
### 2) Enable the gateway intents ClawX needs
...
Discord uses numeric ids everywhere; ClawX config prefers ids.
...
- Owner hint: when a per-guild or per-channel `users` allowlist matches the sender, ClawX treats that sender as the owner in the system prompt. For a global owner across channels, set `commands.ownerAllowFrom`.
...
- First: run `clawx doctor` and `clawx channels status --probe` (actionable warnings + quick audits).
...
When enabled, ClawX uses the member identity for allowlists and labels the
...
- On startup, ClawX resolves channel/user names in allowlists to IDs (when the bot can search members)
  and logs the mapping; unresolved entries are kept as typed.

Native command notes:

- The registered commands mirror ClawX’s chat commands.
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; ClawX enforces allowlists on execution and replies “not authorized”.
...
- If the bot is stuck or rate limited, restart the gateway (`clawx gateway --force`) after confirming no other processes own the Discord session.
