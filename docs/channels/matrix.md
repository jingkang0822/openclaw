...
Matrix is an open, decentralized messaging protocol. ClawX connects as a Matrix **user**
...
```bash
clawx plugins install @clawx/matrix
```

Local checkout (when running from a git repo):

```bash
clawx plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `clawx plugins install @clawx/matrix`
   - From a local checkout: `clawx plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: ClawX calls the same
     login endpoint, stores the access token in `~/.clawx/credentials/matrix/credentials.json`,
...
- On first connection, ClawX requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  ClawX logs a warning.
...
`~/.clawx/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `clawx pairing list matrix`
  - `clawx pairing approve matrix <CODE>`
...
- On startup, ClawX resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
