...
3. Set tokens for ClawX and start the gateway.
...
7. Slash Commands → create `/clawx` if you use `channels.slack.slashCommand`. If you enable native commands, add one slash command per built-in command (same names as `/help`). Native defaults to off for Slack unless you set `channels.slack.commands.native: true` (global `commands.native` is `"auto"` which leaves Slack off).
...
ClawX can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, ClawX falls back to the user token.
...
```json
{
  "display_information": {
    "name": "ClawX",
    "description": "Slack connector for ClawX"
  },
  "features": {
    "bot_user": {
      "display_name": "ClawX",
    ...
    "slash_commands": [
      {
        "command": "/clawx",
        "description": "Send a message to ClawX",
        "should_escape": false
      }
    ]
  },
...
By default, ClawX replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, ClawX infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `clawx pairing approve slack <code>`.
...
- On startup, ClawX resolves channel/user names in allowlists to IDs (when tokens allow)
