...
read_when:
  - Setting up Twitch chat integration for ClawX
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. ClawX connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
clawx plugins install @clawx/twitch
```

Local checkout (when running from a git repo):

```bash
clawx plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `CLAWX_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "clawx", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use CLAWX_TWITCH_ACCESS_TOKEN env var)
...
```bash
CLAWX_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
