...
The onboarding wizard is the **recommended** way to set up ClawX on macOS,
...
`clawx dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
(`web_fetch` works without a key). Easiest path: `clawx configure --section web`
...
If the config is invalid or contains legacy keys, the wizard asks you to run `clawx doctor` first.
</Note>
...
Use `clawx agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.clawx/workspace-<agentId>`.
