...
read_when:
  - You need detailed behavior for clawx onboard
...
This page is the full reference for `clawx onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.clawx/clawx.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `clawx doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `clawx pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `clawx health`.
    - `clawx status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.clawx/.env` so launchd can read it.
...
- OAuth credentials: `~/.clawx/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.clawx/agents/<agentId>/agent/auth-profiles.json`
...
- Command reference: [`clawx onboard`](/cli/onboard)
