...
Deploy ClawX on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/clawx/clawx)
...
repository configures everything needed to run ClawX:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: CLAWX_STATE_DIR
        value: /data/.clawx
      - key: CLAWX_WORKSPACE_DIR
        value: /data/workspace
      - key: CLAWX_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/clawx`.
...
If you use the original ClawX repository, Render will not auto-deploy your ClawX. To update it, run a manual Blueprint sync from the dashboard.
...
For ClawX, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any ClawX host.
