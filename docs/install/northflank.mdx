...
Deploy ClawX on Northflank with a one-click template and finish setup in your browser.
...
1. Click [Deploy ClawX](https://northflank.com/stacks/deploy-clawx) to open the template.
2. Create an [account on Northflank](https://app.northflank.com/signup) if you don’t already have one.
3. Click **Deploy ClawX now**.
4. Set the required environment variable: `SETUP_PASSWORD`.
5. Click **Deploy stack** to build and run the ClawX template.
6. Wait for the deployment to complete, then click **View resources**.
7. Open the ClawX service.
8. Open the public ClawX URL and complete setup at `/setup`.
9. Open the Control UI at `/clawx`.

## What you get

- Hosted ClawX Gateway + Control UI
...
4. Click **Run setup**.
5. Open the Control UI at `https://<your-northflank-domain>/clawx`
