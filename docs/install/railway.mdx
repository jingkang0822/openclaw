...
Deploy ClawX on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/clawx` — Control UI
...
- `CLAWX_STATE_DIR=/data/.clawx` (recommended)
- `CLAWX_WORKSPACE_DIR=/data/workspace` (recommended)
- `CLAWX_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your ClawX state + workspace so you can migrate to another host without losing config or memory.
