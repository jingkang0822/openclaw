...
LINE 通过 LINE Messaging API 连接到 ClawX。该插件作为 webhook 接收器在 Gateway 网关上运行，使用你的 channel access token + channel secret 进行身份验证。
...
```bash
clawx plugins install @clawx/line
```

本地检出（从 git 仓库运行时）：

```bash
clawx plugins install ./extensions/line
```
...
```bash
clawx pairing list line
clawx pairing approve line <CODE>
```
