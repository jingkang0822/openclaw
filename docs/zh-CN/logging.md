...
你可以在 `~/.clawx/clawx.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/clawx.log"
  }
}
```
...
```bash
clawx channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.clawx/clawx.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/clawx/clawx-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- ClawX 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
CLAWX_DIAGNOSTICS=telegram.http,telegram.payload
```
...
- 你也可以使用 `clawx plugins enable diagnostics-otel` 启用插件。
...
- `clawx.tokens`（计数器，属性：`clawx.token`、`clawx.channel`、`clawx.provider`、`clawx.model`）
- `clawx.cost.usd`（计数器，属性：`clawx.channel`、`clawx.provider`、`clawx.model`）
- `clawx.run.duration_ms`（直方图，属性：`clawx.channel`、`clawx.provider`、`clawx.model`）
- `clawx.context.tokens`（直方图，属性：`clawx.context`、`clawx.channel`、`clawx.provider`、`clawx.model`）

消息流：

- `clawx.webhook.received`（计数器，属性：`clawx.channel`、`clawx.webhook`）
- `clawx.webhook.error`（计数器，属性：`clawx.channel`、`clawx.webhook`）
- `clawx.webhook.duration_ms`（直方图，属性：`clawx.channel`、`clawx.webhook`）
- `clawx.message.queued`（计数器，属性：`clawx.channel`、`clawx.source`）
- `clawx.message.processed`（计数器，属性：`clawx.channel`、`clawx.outcome`）
- `clawx.message.duration_ms`（直方图，属性：`clawx.channel`、`clawx.outcome`）

队列 + 会话：

- `clawx.queue.lane.enqueue`（计数器，属性：`clawx.lane`）
- `clawx.queue.lane.dequeue`（计数器，属性：`clawx.lane`）
- `clawx.queue.depth`（直方图，属性：`clawx.lane` 或 `clawx.channel=heartbeat`）
- `clawx.queue.wait_ms`（直方图，属性：`clawx.lane`）
- `clawx.session.state`（计数器，属性：`clawx.state`、`clawx.reason`）
- `clawx.session.stuck`（计数器，属性：`clawx.state`）
- `clawx.session.stuck_age_ms`（直方图，属性：`clawx.state`）
- `clawx.run.attempt`（计数器，属性：`clawx.attempt`）

### 导出的 span（名称 + 关键属性）

- `clawx.model.usage`
  - `clawx.channel`、`clawx.provider`、`clawx.model`
  - `clawx.sessionKey`、`clawx.sessionId`
  - `clawx.tokens.*`（input/output/cache_read/cache_write/total）
- `clawx.webhook.processed`
  - `clawx.channel`、`clawx.webhook`、`clawx.chatId`
- `clawx.webhook.error`
  - `clawx.channel`、`clawx.webhook`、`clawx.chatId`、`clawx.error`
- `clawx.message.processed`
  - `clawx.channel`、`clawx.outcome`、`clawx.chatId`、`clawx.messageId`、`clawx.sessionKey`、`clawx.sessionId`、`clawx.reason`
- `clawx.session.stuck`
  - `clawx.state`、`clawx.ageMs`、`clawx.queueDepth`、`clawx.sessionKey`、`clawx.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `clawx doctor`。
