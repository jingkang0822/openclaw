...
- **会话存储**（聊天历史 + 路由状态）位于 `~/.clawx/agents/<agentId>/sessions` 下。
...
Skills 通过每个工作区的 `skills/` 文件夹实现每智能体独立，共享的 Skills 可从 `~/.clawx/skills` 获取。参见 [Skills：每智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。
...
- 状态目录：`~/.clawx`（或 `CLAWX_STATE_DIR`）
- 工作区：`~/.clawx/workspace`（或 `~/.clawx/workspace-<agentId>`）
- 智能体目录：`~/.clawx/agents/<agentId>/agent`（或 `agents.list[].agentDir`）
- 会话：`~/.clawx/agents/<agentId>/sessions`

### 单智能体模式（默认）

如果你什么都不做，ClawX 运行单个智能体：

- `agentId` 默认为 **`main`**。
- 会话键为 `agent:main:<mainKey>`。
- 工作区默认为 `~/.clawx/workspace`（或当设置了 `CLAWX_PROFILE` 时为 `~/.clawx/workspace-<profile>`）。
- 状态默认为 `~/.clawx/agents/main/agent`。
...
```js
{
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // 可选覆盖。默认：~/.clawx/credentials/whatsapp/personal
