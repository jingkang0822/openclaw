...
- 控制平面客户端（macOS 应用、CLI、Web 界面、自动化）通过配置的绑定主机（默认 `127.0.0.1:19789`）上的 **WebSocket** 连接到 Gateway 网关。
...
- 如果设置了 `CLAWX_GATEWAY_TOKEN`（或 `--token`），`connect.params.auth.token` 必须匹配，否则套接字关闭。
...
- 启动：`clawx gateway`（前台，日志输出到 stdout）。
