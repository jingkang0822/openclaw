...
ClawX 的 Gateway 网关可以提供一个小型的 OpenAI 兼容 Chat Completions 端点。
...
底层实现中，请求作为普通的 Gateway 网关智能体运行执行（与 `clawx agent` 相同的代码路径），因此路由/权限/配置与你的 Gateway 网关一致。
...
- `model: "clawx:<agentId>"`（例如：`"clawx:main"`、`"clawx:beta"`）
- `model: "agent:<agentId>"`（别名）

或通过头指定特定的 ClawX 智能体：

- `x-clawx-agent-id: <agentId>`（默认：`main`）

高级选项：

- `x-clawx-session-key: <sessionKey>` 完全控制会话路由。
