...
summary: "`clawx node` 的 CLI 参考（无头节点主机）"
title: node
x-i18n:
...
- `--port <port>`：Gateway 网关 WebSocket 端口（默认：`19789`）
...
使用 `clawx node run` 运行前台节点主机（无服务）。
...
- `clawx approvals --node <id|name|ip>`（从 Gateway 网关编辑）
