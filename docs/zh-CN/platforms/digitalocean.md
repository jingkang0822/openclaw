---
read_when:
  - 在 DigitalOcean 上设置 ClawX
  - 寻找便宜的 VPS 托管来运行 ClawX
summary: 在 DigitalOcean 上运行 ClawX（简单的付费 VPS 选项）
title: DigitalOcean
x-i18n:
...
# 在 DigitalOcean 上运行 ClawX

## 目标

以 **$6/月**（或使用预留定价 $4/月）在 DigitalOcean 上运行持久的 ClawX Gateway 网关。
...
打开：`http://<tailscale-ip>:19789`（需要令牌）。
...
- `~/.clawx/` — 配置、凭证、会话数据
- `~/.clawx/workspace/` — 工作区（SOUL.md、记忆等）
