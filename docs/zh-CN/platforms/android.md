...
Android 直接连接到 Gateway 网关 WebSocket（默认 `ws://<host>:19789`）并使用 Gateway 网关拥有的配对。
...
- 你可以在 Gateway 网关机器上运行 CLI（`clawx`）（或通过 SSH）。
...
- 在 Gateway 网关主机的 `~/.clawx/clawx.json` 中设置 `gateway.bind: "tailnet"`。
...
1. 在 Gateway 网关主机上设置 DNS-SD 区域（示例 `clawx.internal.`）并发布 `_clawx-gw._tcp` 记录。
...
1. 在 Gateway 网关主机上创建 `~/.clawx/workspace/canvas/index.html`。
...
Tailnet（可选）：如果两个设备都在 Tailscale 上，使用 MagicDNS 名称或 tailnet IP 而不是 `.local`，例如 `http://<gateway-magicdns>:18793/__clawx__/canvas/`。

此服务器将实时重载客户端注入 HTML 并在文件更改时重新加载。
A2UI 主机位于 `http://<gateway-host>:18793/__clawx__/a2ui/`。
