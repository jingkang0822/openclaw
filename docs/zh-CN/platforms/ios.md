...
- 网络路径：
  ...
  - 通过单播 DNS-SD 的 Tailnet（示例域：`clawx.internal.`），**或**
  - 手动主机/端口（备选）。
...
Gateway 网关在 `local.` 上广播 `_clawx-gw._tcp`。iOS 应用会自动列出这些。

### Tailnet（跨网络）

如果 mDNS 被阻止，使用单播 DNS-SD 区域（选择一个域；示例：`clawx.internal.`）和 Tailscale 分割 DNS。
参见 [Bonjour](/gateway/bonjour) 了解 CoreDNS 示例。

### 手动主机/端口

在设置中，启用**手动主机**并输入 Gateway 网关主机 + 端口（默认 `19789`）。
...
- Gateway 网关 canvas 主机服务于 `/__clawx__/canvas/` 和 `/__clawx__/a2ui/`。
...
- 配对提示从未出现：运行 `clawx nodes pending` 并手动批准。
