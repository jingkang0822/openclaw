...
clawx system heartbeat last
clawx logs --follow
clawx config get agents.defaults.heartbeat
clawx channels status --probe
```
...
```bash
clawx config get agents.defaults.heartbeat.activeHours
clawx config get agents.defaults.heartbeat.activeHours.timezone
clawx config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
clawx cron list
clawx logs --follow
```
