...
- Jobs persist under `~/.clawx/cron/` so restarts don’t lose schedules.
...
clawx cron list
clawx cron run <job-id>
clawx cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.clawx/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `clawx cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, ClawX defaults to `announce`.
...
- Job store: `~/.clawx/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.clawx/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
clawx cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
clawx cron edit <jobId> --agent ops
clawx cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
clawx cron run <jobId>
clawx cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
clawx cron edit <jobId> \
...
```bash
clawx system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`clawx system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `CLAWX_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- ClawX applies exponential retry backoff for recurring jobs after consecutive errors:
