---
summary: "ClawX on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up ClawX on a Raspberry Pi
  - Running ClawX on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# ClawX on Raspberry Pi

## Goal

Run a persistent, always-on ClawX Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
ssh -L 19789:localhost:19789 user@gateway-host
...
Most ClawX features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled clawx

# Enable if not
sudo systemctl enable clawx

# Start on boot
sudo systemctl start clawx
```
...
```bash
# Check logs
journalctl -u clawx --no-pager -n 100

# Common fix: rebuild
cd ~/clawx  # if using hackable install
npm run build
sudo systemctl restart clawx
```
