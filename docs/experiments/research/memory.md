...
read_when:
  - Designing workspace memory (~/.clawx/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep ClawX integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.clawx/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.clawx/workspace` as canonical human-readable memory.
...
```
~/.clawx/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in ClawX**, but keep a separable core library.

### Why integrate into ClawX?
...
- You want the agent itself to call the tools:
  - `clawx memory recall "…" --k 25 --since 30d`
  - `clawx memory reflect --since 7d`
...
Pragmatic take for `~/.clawx/workspace`:
