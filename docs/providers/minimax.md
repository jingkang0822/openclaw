---
summary: "Use MiniMax M2.1 in ClawX"
read_when:
  - You want MiniMax models in ClawX
...
```bash
clawx plugins enable minimax-portal-auth  # skip if already loaded.
clawx gateway restart  # restart if gateway is already running
clawx onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/clawx/clawx/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `clawx.json`:
...
- Use `clawx models list` and `clawx models set minimax/MiniMax-M2.1` to switch.
...
- Running `clawx configure` and selecting **MiniMax M2.1**, or
