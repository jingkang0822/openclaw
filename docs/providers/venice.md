---
summary: "Use Venice AI privacy-focused models in ClawX"
read_when:
  - You want privacy-focused inference in ClawX
...
```bash
clawx onboard --auth-choice venice-api-key
```
...
```bash
clawx chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, ClawX shows all available Venice models. Pick based on your needs:
...
```bash
clawx models set venice/claude-opus-45
clawx models set venice/llama-3.3-70b
```

List all available models:

```bash
clawx models list | grep venice
```

## Configure via `clawx configure`

1. Run `clawx configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
ClawX automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
clawx chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
clawx chat --model venice/claude-opus-45

# Use uncensored model
clawx chat --model venice/venice-uncensored

# Use vision model with image
clawx chat --model venice/qwen3-vl-235b-a22b

# Use coding model
clawx chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `clawx models list` to see currently available models. Some models may be temporarily offline.
