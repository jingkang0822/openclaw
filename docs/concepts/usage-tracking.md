...
- `/usage cost` in chats: local cost summary aggregated from ClawX session logs.
- CLI: `clawx status --usage` prints a full per-provider breakdown.
- CLI: `clawx channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
