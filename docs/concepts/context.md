...
read_when:
  - You want to understand what “context” means in ClawX
...
“Context” is **everything ClawX sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (ClawX-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How ClawX builds the system prompt

The system prompt is **ClawX-owned** and rebuilt each run. It includes:
...
By default, ClawX injects a fixed set of workspace files (if present):
