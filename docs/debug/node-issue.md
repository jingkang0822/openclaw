...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in ClawX
...
Running ClawX via Node with `tsx` fails at startup with:

```
[clawx] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `clawx status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch clawx.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node clawx.mjs status` works on Node 25.
