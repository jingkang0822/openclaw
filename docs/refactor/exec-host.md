...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.clawx/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.clawx/node.json` (node id + pairing token).
...
Path: `~/.clawx/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.clawx/exec-approvals.sock",
...
- Unix socket at `~/.clawx/exec-approvals.sock` (0600).
