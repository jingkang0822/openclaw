...
- Publish as `clawx/plugin-sdk` (or export from core under `clawx/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `ClawXPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: ClawXConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: ClawXConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: ClawXConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: ClawXConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: ClawXConfig): string;
  };
};
```
...
- Introduce `clawx/plugin-sdk`.
- Add `api.runtime` to `ClawXPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `clawxRuntime: ">=2026.2.0"`).
