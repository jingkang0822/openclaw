...
ClawX’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`clawx agent`), so routing/permissions/config match your Gateway.
...
```bash
curl -N http://127.0.0.1:19789/v1/responses \
  -H 'Authorization: Bearer YOUR_TOKEN' \
  -H 'Content-Type: application/json' \
  -H 'x-clawx-agent-id: main' \
  -d '{
