...
- During heartbeat runs, ClawX treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `clawx sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, ClawX skips the heartbeat run entirely (no model call).
...
```bash
clawx system event --text "Check for urgent follow-ups" --mode now
```
