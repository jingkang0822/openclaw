...
## Quick check: `clawx security audit`
...
```bash
clawx security audit
clawx security audit --deep
clawx security audit --fix
```
...
- Tighten local perms (`~/.clawx` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... _spicy_. Here’s how to not get pwned.

ClawX is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, ClawX also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.clawx/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.clawx/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.clawx/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.clawx/credentials/oauth.json`
...
`clawx security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${CLAWX_GATEWAY_PASSWORD}
```
...
ClawX stores session transcripts on disk under `~/.clawx/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.clawx` (see the audit section below). If you need
...
ClawX can refresh the skills list mid-session:
...
- If you install plugins from npm (`clawx plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.clawx/extensions/<pluginId>/` (or `$CLAWX_STATE_DIR/extensions/<pluginId>/`).
  - ClawX uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
clawx pairing approve <channel> <code>
```
...
By default, ClawX routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
ClawX has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.clawx/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.clawx or your logs.”
...
4. **Re-run audit**
   - `clawx security audit --deep` and confirm the report is clean.
...
- `~/.clawx/clawx.json`: `600` (user read/write only)
- `~/.clawx`: `700` (user only)

`clawx doctor` can warn and offer to tighten these permissions.
...
- Default: `19789`
- Config/flags/env: `gateway.port`, `--port`, `CLAWX_GATEWAY_PORT`
...
The Gateway broadcasts its presence via mDNS (`_clawx-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `CLAWX_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `clawx doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `CLAWX_GATEWAY_PASSWORD`).

Rotation checklist (token/password):

1. Generate/set a new secret (`gateway.auth.token` or `CLAWX_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), ClawX
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. ClawX verifies the identity by resolving the
...
- ClawX will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.clawx/` (or `$CLAWX_STATE_DIR/`) may contain secrets or private data:

- `clawx.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `clawx status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@clawx", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.clawx/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `clawx` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only ClawX clients can connect.
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `clawx gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `CLAWX_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/clawx/clawx-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.clawx/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + ClawX version
...
Found a vulnerability in ClawX? Please report responsibly:

1. Email: [security@clawx.ai](mailto:security@clawx.ai)
