...
- Replaces the legacy `gateway` command. CLI entry point: `clawx gateway`.
...
- Config hot reload watches `~/.clawx/clawx.json` (or `CLAWX_CONFIG_PATH`).
  - Default mode: `gateway.reload.mode="hybrid"` (hot-apply safe changes, restart on critical).
  - Hot reload uses in-process restart via **SIGUSR1** when needed.
  - Disable with `gateway.reload.mode="off"`.
- Binds WebSocket control plane to `127.0.0.1:<port>` (default 19789).
- The same port also serves HTTP (control UI, hooks, A2UI). Single-port multiplex.
  - OpenAI Chat Completions (HTTP): [`/v1/chat/completions`](/gateway/openai-http-api).
  - OpenResponses (HTTP): [`/v1/responses`](/gateway/openresponses-http-api).
  - Tools Invoke (HTTP): [`/tools/invoke`](/gateway/tools-invoke-http-api).
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__clawx__/canvas/` from `~/.clawx/workspace/canvas`. Disable with `canvasHost.enabled=false` or `CLAWX_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `CLAWX_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `CLAWX_GATEWAY_PORT` > `gateway.port` > default `19789`.
...
- Clients then connect to `ws://127.0.0.1:19789` through the tunnel.
...
- macOS: `bot.molt.<profile>` (legacy `com.clawx.*` may still exist)
- Linux: `clawx-gateway-<profile>.service`
- Windows: `ClawX Gateway (<profile>)`

Install metadata is embedded in the service config:

- `CLAWX_SERVICE_MARKER=clawx`
- `CLAWX_SERVICE_KIND=gateway`
- `CLAWX_SERVICE_VERSION=<version>`
...
```bash
clawx --dev setup
clawx --dev gateway --allow-unconfigured
...
- `CLAWX_STATE_DIR=~/.clawx-dev`
- `CLAWX_CONFIG_PATH=~/.clawx-dev/clawx.json`
- `CLAWX_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.clawx/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `CLAWX_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `CLAWX_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `CLAWX_CONFIG_PATH`
- unique `CLAWX_STATE_DIR`
...
```bash
CLAWX_CONFIG_PATH=~/.clawx/a.json CLAWX_STATE_DIR=~/.clawx-a clawx gateway --port 19001
CLAWX_CONFIG_PATH=~/.clawx/b.json CLAWX_STATE_DIR=~/.clawx-b clawx gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `clawx health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `clawx`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `clawx gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.clawx.*` is cleaned up).
  - `clawx doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are ClawX profile services.
  ...
  - Cleanup: `clawx gateway uninstall` (current service) and `clawx doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `clawx gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- ClawX.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.clawx.*` labels still unload cleanly).
- To stop it cleanly, use `clawx gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `clawx gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `clawx gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

ClawX installs a **systemd user service** by default on Linux/WSL2. We
...
`clawx gateway install` writes the user unit. `clawx doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/clawx-gateway[-<profile>].service`:

```
[Unit]
Description=ClawX Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/clawx gateway --port 19789
Restart=always
RestartSec=5
Environment=CLAWX_GATEWAY_TOKEN=
...
```
systemctl --user enable --now clawx-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/clawx-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now clawx-gateway[-<profile>].service
```
...
- `clawx gateway health|status` — request health/status over the Gateway WS.
- `clawx message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `clawx agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `clawx gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `clawx gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `clawx gateway` and the legacy TCP control port.
