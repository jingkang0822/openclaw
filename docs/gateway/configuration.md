---
summary: "All configuration options for ~/.clawx/clawx.json with examples"
read_when:
...
ClawX reads an optional **JSON5** config from `~/.clawx/clawx.json` (comments + trailing commas allowed).

If the file is missing, ClawX uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.clawx/workspace`). You usually only need a config to:
...
ClawX only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `clawx doctor`, `clawx logs`, `clawx health`, `clawx status`, `clawx service`, `clawx help`).
- Run `clawx doctor` to see the exact issues.
- Run `clawx doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `clawx config set`. Keep a backup of `~/.clawx/clawx.json`.
...
```bash
clawx gateway call config.get --params '{}' # capture payload.hash
clawx gateway call config.apply --params '{
...
```bash
clawx gateway call config.get --params '{}' # capture payload.hash
clawx gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.clawx/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@clawx", "reisponde"] },
      },
    ],
  },
  channels: {
...
// ~/.clawx/agents.json5
{
  ...
  list: [{ id: "main", workspace: "~/.clawx/workspace" }],
}
```
...
{ "$include": "/etc/clawx/base.json5" } // absolute
...
// ~/.clawx/clawx.json
{
  gateway: { port: 19789, auth: { token: "secret" } },
...
```json5
// ~/.clawx/clients/mueller/agents.json5
[
...
```json5
// ~/.clawx/clients/mueller/broadcast.json5
{
...
ClawX reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, ClawX runs your login shell and imports only the missing expected keys (never overrides).
...
- `CLAWX_LOAD_SHELL_ENV=1`
- `CLAWX_SHELL_ENV_TIMEOUT_MS=15000`
...
ClawX stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.clawx/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.clawx/credentials/oauth.json` (or `$CLAWX_STATE_DIR/credentials/oauth.json`)
...
- `~/.clawx/agent/*` (migrated by `clawx doctor` into `~/.clawx/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `CLAWX_OAUTH_DIR`
- Agent dir (default agent root override): `CLAWX_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, ClawX imports `oauth.json` entries into `auth-profiles.json`.
...
If set, ClawX derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/clawx/clawx-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/clawx/clawx.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.clawx/credentials/whatsapp/biz
          // authDir: "~/.clawx/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `clawx doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@clawx", "clawx"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@clawx"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.clawx/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.clawx/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.clawx/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.clawx/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.clawx/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.clawx/workspace-home" },
      { id: "work", workspace: "~/.clawx/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `clawx.json`).
...
ClawX starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["clawx-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-clawx",
...
ClawX starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `clawx plugins install @clawx/mattermost` (or `./extensions/mattermost` from a git checkout).
...
ClawX starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
ClawX spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, ClawX
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/clawx" } },
}
```
...
When a file exceeds this limit, ClawX logs a warning and injects a truncated
...
message envelopes). If unset, ClawX uses the host timezone at runtime.
...
`[clawx]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[clawx]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[clawx]"`, ClawX will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, ClawX generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.clawx/settings/tts.json",
      elevenlabs: {
...
ClawX also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, ClawX currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `clawx configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:clawx`: all built-in ClawX tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.clawx/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.clawx/sandboxes",
        docker: {
          image: "clawx-sandbox:bookworm-slim",
          containerPrefix: "clawx-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
        ...
        browser: {
          enabled: false,
          image: "clawx-sandbox-browser:bookworm-slim",
          containerPrefix: "clawx-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
ClawX uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.clawx/agents/<agentId>/agent/models.json` or by defining the same schema inside your
ClawX config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, ClawX writes/merges a `models.json` into
`~/.clawx/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. ClawX uses
...
- Shortcut: `clawx onboard --auth-choice opencode-zen`.
...
Shortcut: `clawx onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `clawx onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `clawx onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `clawx onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `clawx onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `clawx onboard --auth-choice minimax-api`.
...
- Override the agent config root with `CLAWX_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.clawx/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.clawx/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.clawx/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, ClawX stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.clawx/skills` (workspace skills
...
from `~/.clawx/extensions`, `<workspace>/.clawx/extensions`, plus any
...
### `browser` (clawx-managed browser)

ClawX can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for clawx and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (ClawX.app menubar, or `clawx gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      clawx: { cdpPort: 18800, color: "#FF4500" },
...
- bind: `loopback`
- port: `19789` (single port for WS + HTTP)

```json5
{
  gateway: {
    mode: "local", // or "remote"
    port: 19789, // WS + HTTP multiplex
    bind: "loopback",
...
- When a connection comes from one of these IPs, ClawX uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `clawx gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `CLAWX_GATEWAY_PORT` > `gateway.port` > default `19789`.
...
- `gateway.auth.password` can be set here, or via `CLAWX_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. ClawX
...
- `gateway.remote.transport` selects the macOS remote transport (`ssh` default, `direct` for ws/wss). When `direct`, `gateway.remote.url` must be `ws://` or `wss://`. `ws://host` defaults to port `19789`.
- `gateway.remote.token` supplies the token for remote calls (leave unset for no auth).
- `gateway.remote.password` supplies the password for remote calls (leave unset for no auth).

macOS app behavior:

- ClawX.app watches `~/.clawx/clawx.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.clawx/clawx.json` (or `CLAWX_CONFIG_PATH`) and applies changes automatically.
...
- `~/.clawx/clawx.json` (or `CLAWX_CONFIG_PATH`)
...
- `CLAWX_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `clawx --dev …` → uses `~/.clawx-dev` + shifts ports from base `19001`
- `clawx --profile <name> …` → uses `~/.clawx-<name>` (port via config/env/flags)
...
```bash
CLAWX_CONFIG_PATH=~/.clawx/a.json \
CLAWX_STATE_DIR=~/.clawx-a \
...
Gmail helper config (used by `clawx webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "clawx@gmail.com",
...
- Set `CLAWX_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, ClawX defaults `serve.path` to `/` so
...
Default root: `~/.clawx/workspace/canvas`  
Default port: `18793` (chosen to avoid the clawx browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__clawx__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__clawx__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
- The Gateway could expose a simple TCP bridge for nodes (iOS/Android), typically on port `19790`.
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.clawx/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.clawx/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.clawx/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_clawx-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `clawx` (advertises `clawx.local`). Override with `CLAWX_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_clawx-gw._tcp` under `~/.clawx/dns/` using the configured discovery domain (example: `clawx.internal.`).
