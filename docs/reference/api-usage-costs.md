...
ClawX features that can generate provider usage or paid API calls.
...
- `clawx status --usage` and `clawx channels list` show provider **usage windows**
...
ClawX can pick up credentials from:
...
`clawx models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
