...
ClawX integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- ClawX owns lifecycle when `channels.signal.autoStart=true`.
...
- ClawX spawns `imsg rpc` as a child process (legacy iMessage integration).
